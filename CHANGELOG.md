...
- **D7C source-bound evidence verification 文档与动态计数**：以仓库外受控 coverage/provenance artifact 实测并绑定当前 HEAD 的 23 schemas / 36 CLI scripts / 35 exit-2 scripts / 55 test files / 910 tests / 910 passed / 0 failed / success=true；self-test 为 260/260，samples coverage 为 280 fixtures、242 个文件引用、15 个目录引用、0 未登记。登记 `evidence-provenance.schema.json`、`wm-verify-evidence-source.ts` 与 source-bound/package-only 边界，明确受控本机 provenance 不是密码学签名或第三方不可抵赖证明；不改变 D7A/B 逻辑、Schema 内容、package 行为、hook 控制流、baseline 或计划/spec。
