...
## 收尾：整分支最终审查、修复波与延后项裁定

> 本节由编排者在 P1 完成后追加（SDD 收尾）。账本本体（`.superpowers/sdd/...`）是 gitignored 的临时工作区，删除后不再存在，故把**裁定结论**固化在此。

### 1. 最终整分支审查（15 提交，228b1168..4f103adb）

结论：**修完再合**。发现 1 条 Critical + 3 条 Important——**全部是单任务审查看不见的跨任务问题**：

| 级别 | 发现 | 性质 |
| --- | --- | --- |
| **C1** | M15 新增的「人类入口描述只写一行、去掉触发清单」与同分支 S02 落地的 `SKILL.md` description（**恰好是一份触发清单**）**互斥**：按 M15 执行就要删掉触发措辞 → 推翻 AC-4 并危及技能自动激活 | 跨任务规则冲突（M15 的 `human-facing description` 前提是外部 harness 有 `disable-model-invocation` 的斜杠命令列表；本仓库没有该前提，frontmatter 就是模型激活面） |
| **I1** | `asset-authoring.md` §6 把源文 S03 的「引用**其他技能**」译成「引用其他**资产**」，变成一条全仓零使用、且与既有 references 整体对立的硬性标记规则 | 采纳期 scope 漂移 |
| **I2** | `asset-authoring.md` §5「引用只允许一层深：**所有**参考文件都由 `SKILL.md` 链接」与仓库既有 **2 跳**架构冲突（7 个文件在 `SKILL.md` 中出现 0 次），而 AC-2 却声称该规则能解释既有加载策略 | 新规则与既有资产对账缺失（任务 1 与任务 2 未对账） |
| **I3** | `README.md:37` 的绝对声明「**与仓库内其他文档**…一律指向、不复制命令」被 M16 **自己的落点文件**证伪（`INSTALL §4/§8`、`CONTRIBUTING`） | 交付物对读者失真 |

### 2. 修复波（1 次，提交 `e2bc50a5`）

按 SDD「一个修复子智能体带完整发现清单」执行一次修复，另含 8 项顺带 Minor。修复后 `relativeLinkCount` 由 671 → **670**（合并 `SKILL.md:75` 同行双链接），已按既有流程重基线 `l0-baseline.ts` 并加理由注释。

### 3. 定向复审（1 次，恰好一次）

结论：**C1 / I1 / I2 / I3 与顺带项 ①-⑧ 全部 ADDRESSED，无新的 Critical/Important 破坏**。独立复核要点：`SKILL.md:4-9` 的 description **仍是完整触发清单**（AC-4 未被推翻，C1 两处文字已一致）；改后的 §5 已把「新增文件」与「既有 2 跳文件例外」分开且覆盖那 7 个文件；`audit:l0-links` 独立复跑得 670/violations `[]`；D-1 边界独立复核 `hard-constraints.md` 相对 base 为 `23 增 / 0 删`。

### 4. 残留项与裁定（熔断规则：Minor 不进循环，无第二波修复）

复审报出的 4 条**新引入 Minor** 与 3 条范围外观察，逐条裁定：

| # | 位置 | 裁定 | 理由 |
| --- | --- | --- | --- |
| R1 | `CONTRIBUTING.md:20` | **搁置（承认真实，延后）** | 机械合并残留：注释出现重复从句 + 悬空冒号，且在被声明「不复述安装命令」的同一块内仍写出 `npm install` 字样。纯文本瑕疵，不影响可执行性 |
| R2 | `CONTRIBUTING.md:17` | **搁置** | 「这些命令」在命令被移出后指代悬空 |
| R3 | `asset-authoring.md:76` | **搁置（真实且延后）** | 在声明 `subagent-delegation.md` §3.1 为权威清单的同时内联 11 个文件名 → 形成第二份可漂移副本，与该文件自身的单权威/co-location 原则有轻微张力。**这是真实缺陷**，但它揭示的是"例外清单该指向还是该复制"的取舍，宜由后续任务统一裁决 |
| R4 | `subagent-delegation.md:188` | **搁置** | 表格「描述文本面向」格仍写「人——一行摘要」，与同行「本仓库没有独立的人类入口描述面」并置，仅靠 :191 要点消解，速读表格者可能误读 |
| R5 | `README.md:160` 去重注释 vs `:164 npm run doctor` | **搁置（M16 落点既有残留）** | 去重注释正下方仍出现 `doctor`（权威块三条命令之一） |
| R6 | `CONTRIBUTING.md:63`/`:90` 的 `npm run self-test` | **搁置** | 属贡献者回归步骤 / pre-push 第 1 项语境，不在「两条路线」节内，故收窄后的 `README.md:37` 字面为真 |
| R7 | `w-model-dev/references/quickstart.md:22` 期望值 `322/322` | **搁置（既有漂移，非本分支引入）** | 当前基线为 332；本分支未触碰该文件 |

**承重性判定**：以上 7 条**均不承重**——没有后续任务建立在它们之上，也不使交付物不可用；且 R3/R7 的真实缺陷在性质上属"既有或待统一裁决"，把它搁置并留下书面裁定比在无授权下即兴修改更安全（D-6 之外，本分支授权面为 M16 的三文件落点，R1/R2/R6 的 `CONTRIBUTING` 已属授权边缘）。

### 5. 该分支未做的事（显式记录）

- **AC-3 的状态标注**：设计 spec §13 只标注了 AC-2 / AC-4 / AC-11（计划自定范围如此）。AC-3 的**事实**已达成（48 条三列逐字未动、`maxAntiPattern: 48` 与 `#1~#48` 契约不变、不做全量改写的三重理由已写入），**状态标注缺失**。
- **`docs-consistency-logic.ts:1309`/`:231` 的过时注释**（「真实文档为『### 命中高发阶段』」现已是新增子节）：纯注释、零功能影响；P1 为纯 L0 阶段，未在该 `.ts` 内改动。
- **`docs/INSTALL.md` 的验证仓库/校验脚本可用性两节的 PowerShell 形态**：指向 README 权威后，README 权威块只有单个 bash 围栏，PowerShell 逐行写法留在散文里（避免复制命令与保留跨 shell 指引之间的取舍）。
