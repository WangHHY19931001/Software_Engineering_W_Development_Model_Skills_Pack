...
> **注意**：主模板节号**保持现有 1-12 编号体系不变**（§3=User Stories 被 `tla-plus-guide.md`/`tla-spec-template.md` 以 `requirement-spec.md:§3` 跨引用，不可移动）。6 个引用块节追加为 **§13 系统上下文 / §14 核心概念与术语 / §15 需求追踪矩阵 / §16 行为规格模型 / §17 Phase 1 工程纪律与 DoD / 附录 A. UML 需求建模**（§12 RTM 登记保留原编号）。门禁按引用块文件名校验（`[name](./xxx.md)`），不依赖节号。
