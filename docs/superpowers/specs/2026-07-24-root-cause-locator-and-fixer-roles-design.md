...
> **影响范围**：W 模型返工循环全阶段；新增第 6 角色 R；F 由 S 兼任；新增反模式 #18/#19；新增多人格多角度分析机制（并行/串行均可）
...
新增 **根因定位者（R）** 角色，在 V/G 不通过后、S 修复前，强制介入根因定位。修复者（F）由现有 S 兼任，S 携带 R 报告执行修复。引入多人格多角度分析机制（并行/串行均可），利用 `w-model-dev/subagent/` 中 28 个人格文件从多角度诊断——**本质是多角度而非并行**，宿主不支持并行时串行分派等价合法。
...
| **允许动作** | ① 读失败产物文件 + 上游产物（需求/设计/代码/测试/TLA+/graph.json）；② 读 V 的 `VerifierOutput` JSON + G 的 GATE_JSON；③ 运用根因分析方法（5-Why / 鱼骨图 / 缺陷链追溯 / 上游回溯）；④ 产出 `RootCauseReport` JSON + `.md` 报告文件；⑤ 标记 `upstreamDefect`（若根因为上游需求/设计缺陷）；⑥ 作为 R-lead 分派 R-persona 子代理（并行或串行均可）并聚合产出（第 9 节） |
...
| 文件 | 修改内容 |
|---|---|
| `docs/skill-design-document_SSoT.md` | §3.4 角色表新增 R 行；§6.4 新增 R 角色定义节（F 标注 S 兼任）；§6.4.x 新增 root-cause-locator 方法论引用 + 多人格多角度分析机制说明（并行/串行均可）；§10.x 新增 check-rootcause-report.ts 校验项；§4A 反模式清单新增 #18/#19 |
...
| 文件 | 修改内容 |
|---|---|
| `w-model-dev/references/subagent-delegation.md` | 角色表新增 R 行（允许/禁止动作）；新增 R 子代理分派模板；新增 V-rootcause 复审分派模板；新增 S-fix 分派模板；新增 R-lead/V-lead 多角度分派模板（第 9 节，并行/串行均可）；回填契约新增 R/S-fix 返回格式；强制约束新增「跳过 R 命中 #18」；时序图更新；失败模式表新增 R 相关场景 |
...
| `w-model-dev/references/agent-personas.md` | 新增「与 root-cause-locator.md 的关系」节 + 「与 subagent/ 人格库的关系」节 + 多角度分派说明（并行/串行均可）；声明 R 不调用 Persona，Persona 不调用 R |
| `w-model-dev/references/operational-recovery.md` | 成本预算与运行日志节新增 rootcause/fix 动作的 token 计量（串行分派时累计）；CHECKPOINT 放行节新增场景 5 阶段回退说明 |
...
| 新增多角度机制 | 1（R-lead/V-lead 多人格多角度分析，并行/串行/降级均可） |
...
- `check-budget.ts` / `check-maturity.ts` / `check-checkpoint.ts`：不直接受影响（run-log 间接关联；budget 仅新增 R4-A 多角度 R 预算规则）
- `w-model-dev/templates/`：现有模板不变（R 报告模板嵌入 root-cause-locator.md）
- `w-model-dev-demo/`：参考实现，可在后续轮次补 R 循环演示，本次不强制

---

## 9. 多人格多角度分析机制（整合 subagent/ 人格库）
...
**本机制的本质是「多角度」，不是「并行」。** 并行只是性能优化，串行同样合法。

**核心要求**：在强制多角度场景（见 9.11）下，R-lead / V-lead **必须**加载 N 个不同 persona，从 N 个不同视角产出 N 份 PartialReport 并聚合——**不论这 N 个 persona 是同时分派（并行）还是依次分派（串行）**。

**分派方式选择规则**：

| 宿主 Agent 能力 | 分派方式 | 说明 |
|---|---|---|
| 支持并行子代理（如 Trae Task 多调用、Claude 并行 tool use） | **并行分派**（推荐） | N 个 R-persona 同时执行，R-lead 收齐 N 份 PartialReport 后聚合；延迟最低 |
| 仅支持串行子代理 | **串行分派**（合法等价） | R-lead 依次分派 N 个 R-persona，每个产出后立即收集，N 份齐后聚合；延迟较高但产出等价 |
| 单会话无子代理 | **单 R-lead 多轮切换 persona**（降级） | R-lead 自身在 N 轮对话中分别加载 N 个 persona 文件作为视角提示，每轮产出一份 PartialReport；仍须聚合 N 份 |

**关键约束（三种方式均强制）**：
1. **N 份 PartialReport 必须独立产出**：每份须独立承载该 persona 的视角、根因链、证据、confidence，不得合并产出。
2. **聚合规则不变**：9.6 / 9.7 的收敛/仲裁/合并规则对三种方式一视同仁。
3. **PartialReport 归档不变**：不论并行/串行/降级，均须归档到 `.w-model/rootcause/partial/<reportId>/<personaSlice>.json`。
4. **run-log 记录不变**：每份 PartialReport 的产出各记一条 `rootcause` 动作（actor=R-persona:<slice>），聚合记一条 `rootcause` 动作（actor=R-lead）。

### 9.2.1 分派流程示意（并行 / 串行 两版）
...
```
O 分派 R-lead（主 R，加载 root-cause-locator.md + 聚合职责）
  ↓ 依次串行分派 N 个 R-persona 子代理（每个产出后立即收集）
  R-persona-1（incident-response-commander）→ PartialReport-1 → 归档 → 收集
  R-persona-2（code-reviewer）             → PartialReport-2 → 归档 → 收集
  R-persona-3（evidence-collector）         → PartialReport-3 → 归档 → 收集
  R-persona-4（reality-checker）            → PartialReport-4 → 归档 → 收集
  ↓ N 份 PartialReport 齐备
R-lead 聚合 N 份 → 产出最终 RootCauseReport（聚合规则同并行版）
  ↓
O 分派 V-lead 复审根因（同样可串行分派 V-persona）
```

> 串行版与并行版的唯一差异是延迟；产出结构、聚合规则、归档要求、run-log 记录完全一致。

### 9.3 R-persona 选择矩阵（按 rootCause.category 与阶段）

| rootCause.category 候选 | 阶段 | 加载的 R-persona（并行/串行均可） |
|---|---|---|
...
### 9.4 V-persona 选择矩阵（评审多角度）

| 评审场景 | 阶段 | 加载的 V-persona（并行/串行均可） |
|---|---|---|
...
`budget.json` 新增 `rootcauseParallelBudget` 字段（字段名保留向后兼容，实际含义为「每轮多角度 R 的 token 预算」，不论并行/串行均累计）：
...
`check-budget.ts` 新增 R4-A 规则：一轮内所有 R-persona（不论并行或串行分派）的 tokens 之和超 `maxTotalTokensPerRound` → 触发 killSwitch。串行分派时，编排者须在每条 `rootcause` 动作记录中累计 tokens，最终汇总校验。
...
扩展解读（并行与串行均适用）：
- **O 分派 persona 子代理（并行或串行）** = 「命令/用户完成组合」，不违反规则
- **R-lead 依次串行分派 persona 子代理** = 仍是「命令完成组合」，每次分派是独立的命令-子代理调用，不构成 persona 互相调用
- **R-lead / V-lead 聚合 persona 产出** = lead 角色聚合，不是 persona 互相调用
- **persona 子代理之间不通信** = 每个 persona 独立产出 PartialReport，不互相调用（串行分派时，后一个 persona 也不读取前一个 persona 的产出，仅产出自己的 PartialReport）

### 9.11 强制 vs 可选

> 本节的「强制」指**强制多角度**（必须加载 N 个 persona 并聚合），**不要求必须并行**。宿主不支持并行时，串行分派 N 个 persona 同样满足「强制多角度」要求。
...
| 根因报告 V 复审 | **强制多角度** | 根因准确性须多角度保证（并行或串行均可） |
| maxReworkRounds 达上限前一轮 | **强制多角度** | 最后一轮须多角度穷尽（并行或串行均可） |

### 9.12 R-lead 子代理分派模板（多角度变体，并行/串行均可）
...
执行：
  ...
  4. 按聚合规则（9.6）产出最终 RootCauseReport
...
7. **多角度机制**：R-lead/V-lead 多人格多角度分析含选择矩阵 + 聚合规则 + 强制/可选划分 + token 预算 + 并行/串行/降级三种分派方式等价说明。
...
4. ~~并行 persona 子代理的宿主机制~~ **已解决**：第 9.2 节明确多角度是本质，并行只是性能优化。宿主不支持并行时，串行分派（依次分派 N 个 persona 子代理）或单会话降级（R-lead 多轮切换 persona）均为合法等价实现。实施时按宿主能力选择 `dispatchMode ∈ {parallel, serial, degraded}`，并在 R-lead 返回的 `dispatchMode` 字段记录。
