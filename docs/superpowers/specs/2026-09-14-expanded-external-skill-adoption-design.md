...
| AC-6 | M05+M17+S13+S14 后：R 入场要求红信号四项验收与 3–5 条排序可证伪假设；**存在 `noRootCause`/`environmental` 合法分支且 R2/R3 未被放松**；≥3 次失败的技术判据可判定；**角色分离可验证**（信号由 S 建立运行、根因由 R 产出、修复由 S-fix 执行）。**P3 范围内部分达成（2026-09-15，各证据命令均真实跑）**：①「≥3 次失败的技术判据可判定」**已达成**——`w-model-dev/references/root-cause-locator.md` §2.6「≥3 次修复失败：架构判据」给出触发门槛（同一问题连续 ≥3 次修复失败；计数依据 = run-log 既有 `round` / `reworkHints`，不新增字段，`metrics-report.ts` 的 `rework.maxConsecutiveRuns` 可作只读旁证）与三条技术判据（每次修复暴露新的共享状态/耦合/别处问题**且位置不同** / 修复需「大规模重构」才能实施 / 每次修复在别处产生新症状 → 结论「这不是假设失败，是架构错误」= 停下做架构讨论），并明确只以一句话引用既有 `maxReworkRounds`（不逐项列其清单）、不改变 `root-cause-locator` 与 `check-rootcause-report.ts` 的 R2–R5 门禁、不新增 Schema 字段。②「角色分离可验证」的 **P3 侧已达成**——`subagent-delegation.md` §3.4（§3.4.1 禁止编排者预判 findings：`do not flag` / `don't treat X as a defect` / `at most Minor` / `the plan chose` 逐字命中即停下重写 prompt；§3.4.3 逐 finding 输出 `ADDRESSED \| NOT ADDRESSED` 且「Attempted is not addressed」；§3.4.6 **R 前置不变**，scoped re-review 不构成跳过 R 的旁路）与 `verifier-spec.md` §15（§15.1 不信任报告：实现者报告 = 未经核实的 claim、stated rationale 从不降级 severity、测试输出的 warning 即 finding；§15.2 误报质疑通道 `FALSE-POSITIVE-CHALLENGE`——**裁决者 = 新的 V，O 只转达与记录不得裁决**，命中反模式 #10；§15.3 spec 自审四项 + 独立 spec reviewer 五类别 + calibration 阈值），即信号由 S 建立运行、根因由 R 产出、修复由 S-fix 执行，O 侧被禁止预判与裁决。证据命令（实测）：`grep -n '## 2.6\|≥3 次' w-model-dev/references/root-cause-locator.md` → `82:## 2.6 ≥3 次修复失败：架构判据` 等；`grep -c '禁预判\|ADDRESSED' w-model-dev/references/subagent-delegation.md` → **6**；`grep -n '^#### 3\.4\.' w-model-dev/references/subagent-delegation.md` → §3.4.1–§3.4.6 六节齐；`grep -n '^## 15\|^### 15\.\|^#### 15\.4' w-model-dev/references/verifier-spec.md` → `1025:## 15. 评审侧信任、质疑与校准契约（S07 / S08 / S09 / S16）` / `1030:### 15.1 不信任报告（S07）` / `1041:### 15.2 误报质疑通道（S08）` / `1071:### 15.3 spec 自审与评审者校准（S16）` / `1114:#### 15.4 scoped re-review 契约（S09）`；`npm run self-test` → 「总计 340 条用例：340 通过，0 失败」；`npm run audit:l0-links` → exit 0、`violations: []`、672/95/36；`npm run prepush` → **18/18 全绿、`PREPUSH_EXIT=0`**（2026-09-15 收口实测，耗时 21 分 23 秒）。**未达成部分（属 P5：调试、运维与交互，本节不得声明整体达成）**：R 入场的「红信号四项验收 + 3–5 条排序可证伪假设」= **M05** 未落地；S 的 loop 构造方法清单 = **M17** 未落地；`noRootCause` / `environmental` 合法分支与 `check-rootcause-report.ts` R2/R3 的对应契约 = **S13** 未落地（当前 R2/R3 按现契约收紧，未放松，但也因此尚无「无根因」合法出口）。P3 附带记录：本计划唯一新增门禁面为既有契约测试 `examples-contract.test.ts` 抓到的 §3.4.6 段落缺规范短名「普通 V/G 失败链」的回归，已在 P3 收口前修复（commit `1701adfa`，1 文件 +1/−1，修复后该测试 30/30 全绿）；P3 全范围**零 .ts / 零 Schema / 零新增 w-model-dev 资产文件**（`git diff --stat df966a07..HEAD -- 'w-model-dev/**/*.ts' 'w-model-dev/**/*.json'` 为空——同时支撑「零 .ts」与「零 Schema」；`git diff --stat --diff-filter=A df966a07..HEAD -- w-model-dev` 为空——支撑「零新增资产」，该区间内唯一新增文件是本计划文件本身，位于 `docs/superpowers/`）。限定：本条仅记录 P3 已落的 S14 与角色分离侧；P5 落地后须以同一组命令复验并补齐 M05/M17/S13 证据。**P5 补齐后整体达成（2026-09-16，各证据命令均真实跑）**：①**M05（R 入场门）已落地**——`w-model-dev/references/root-cause-locator.md` §2.7「R 入场门（M05）」（`:115`）给出红信号四项验收（**本文件称「五件套」**——同组 5 条判据的准确列数；台账 `2026-09-14-external-repo-capability-survey.md:189` 与 AC-6 冻结文本沿用「四项验收」名义）+ **红得起之前禁进假设**（反锚定）+ 红信号就位后 **3–5 条排序可证伪假设**（预测格式「若 X 是因，则改 Y 让 bug 消失／改 Z 更糟」）+ 造不出红信号 → **停下置 blocked 三选一**（可复现环境访问权／脱敏捕获 artifact／临时生产插桩许可）；证据：`grep -n "^## 2\.7" w-model-dev/references/root-cause-locator.md` → `115:## 2.7 R 入场门（M05）`；`grep -n "四项验收" w-model-dev/references/root-cause-locator.md` → `121:...（红信号四项验收，本文件称「五件套」...`（即别名行，两名义可互 grep）；`grep -c "五件套" w-model-dev/references/root-cause-locator.md` → **2**。②**M17（S 侧 loop 构造方法清单）已落地**——同文件 §2.8（`:147`）10 种方法**按序尝试**（failing test → curl/HTTP 脚本 → CLI + fixture diff → headless browser → replay trace → throwaway harness → property/fuzz → bisection → differential → HITL 兜底），并给出非确定性 bug 判据「**目标不是干净复现，而是提高复现率**」（50% flake 可调试、1% 不可）；证据：`grep -n "^## 2\.8" w-model-dev/references/root-cause-locator.md` → `147:## 2.8 S 侧 loop 构造方法清单（M17）`。③**S13（`noRootCause` 合法分支）已落地且 R2/R3 未被放松**——schema `w-model-dev/schemas/rootcause-report.schema.json`（`noRootCause` 定义 `:107`、顶层 `oneOf` 两支互斥且 `"oneOf"` 出现 **1** 次、`:265` 明示「常规 RootCauseReport：R2/R3 字段保持必填，且不得同时宣告 `noRootCause`」）+ `w-model-dev/scripts/logic/root-cause-logic.ts`（`hasNoRootCause` 分流 `:200`／`:242`，R1/R2/R3 判据**逐字保留**于常规分支）+ 夹具 `w-model-dev/scripts/samples/rootcause/valid-no-root-cause.json` 与 `.../bad-no-root-cause-missing-investigation.json`，文档侧 §2.9（`:172`）；证据（真实退出码）：`npx tsx w-model-dev/scripts/cli/check-rootcause-report.ts w-model-dev/scripts/samples/rootcause/valid-no-root-cause.json` → `ROOTCAUSE_JSON {"type":"rootcause","passed":true,"exitCode":0,"reasonCount":0}`、`EXIT=0`；同一命令指向 `bad-no-root-cause-missing-investigation.json` → `{"passed":false,"exitCode":1,"reasonCount":1}` + `• [schema] /noRootCause: must have required property 'investigation' [required]`、`EXIT=1`；**R2/R3 未放松**：`git diff be1a77a9..HEAD -- w-model-dev/scripts/logic/root-cause-logic.ts` 的命中行**全部为缩进移位**——`git diff -w --ignore-blank-lines` 同一路径对 `MIN_CHAIN_LENGTH`／`MAX_CHAIN_LENGTH`／`FALSIFIABILITY_PATTERN`／「rootCauseChain 长度必须在」四类 token **零增删行**，盘上常量仍为 `MIN_CHAIN_LENGTH = 2`（`:143`）、`MAX_CHAIN_LENGTH = 5`（`:144`）、`FALSIFIABILITY_PATTERN = /若.*则/`（`:149`）；`ls w-model-dev/schemas/*.schema.json` → **34**（零新增 Schema 文件）。④「≥3 次失败的技术判据可判定」——P3 已达成（见本行前段），本次仅复验：`grep -n "^## 2\.6" w-model-dev/references/root-cause-locator.md` → `82:## 2.6 ≥3 次修复失败：架构判据`。**两条如实说明**：(a) S13 的 schema 分支属 **P0 已批准的 C 级产物清单**（规格 `:211`：`noRootCause` / `environmental` 分支（S13）｜`Schema 扩展`｜`新增分支，不放松 R2/R3`），故**不适用 M07 式的单独 Schema 批准**——裁定 D-2（`2026-09-14-external-absorption-adjudication.md:222`）只切出 M07 那一个例外；(b) 本行「红信号四项验收」与 §2.7「五件套」是**同一组 5 条判据的两个名义**（台账 `:189` 与 AC-6 用「四项验收」、§2.7 标「五件套」），本次已在 §2.7 判据行加别名，两名义可互 grep。**P5 一致性证据**：`npm run self-test` → 「总计 344 条用例：344 通过，0 失败」、`EXIT=0`；`npm run audit:l0-links` → exit 0、`relativeLinkCount:672`／`l1OnlyCount:95`／`templatePlaceholderCount:36`、`violations:[]`；`ls w-model-dev/references/*.md` → **43**；`ls w-model-dev/scripts/cli/*.ts` → **46**；「342 条用例」计数**零残留**（裸 `342` 仅命中 sha256 十六进制串与 `run-log-logic.test.ts` 内哈希常量，无计数声明）；`npx tsx w-model-dev/scripts/cli/check-samples-coverage.ts` → `SAMPLES_COVERAGE_JSON {"type":"samples-coverage","fixtureCount":372,"referencedFiles":328,"referencedDirs":15,"unregistered":0,"danglingRefs":0,"undeclaredDirs":0,"negativeCoverageMissing":0,"negativeCoverageDangling":0,"exitCode":0}`、`EXIT=0`；测试门（控制者独占运行，最终树实测）：`npm run prepush` → **18/18 全绿、`PREPUSH_EXIT=0`**，末行「全部门禁通过，允许推送 ✓」（2026-09-16 收口实测，18 项逐项见本轮账本；S24 未进该 18 项、`prePushCount: 18` 未变） |
...
| AC-9 | S23 后 worktree 条目：**"worktree 内 `.w-model` 合法性 + 合并回主仓"规则已单独批准**；隔离五条、清理拥有权判定、`prune` 自愈、clean baseline 强制均已落地；不存在自动 `git reset`、自动清理用户文件或绕过 CHECKPOINT 的路径。**已达成（P5，2026-09-16）**：**规则已单独批准** → 裁定 D-6（`docs/superpowers/plans/2026-09-14-external-absorption-adjudication.md:133` 原则：「worktree 内**禁止**写 `.w-model`；跨 worktree 的状态与证据**一律回主仓**」；`:224` 生效回执：D-6 的落地**阻塞于**「worktree 内 `.w-model` 合法性」规则定稿，未定稿前仅可实施 S23-a 四条工程纪律——该阻塞条件现由下述定稿解除）；**规则定稿落点** → `w-model-dev/references/phase-5-coding.md` 新节「worktree 纪律（S23）」（`:232`，D-6 原则逐字 + 三点依据：新建 worktree **不继承** `.w-model/`、风险是写入位置分叉；`wm-write.ts` 锁在同一文件路径上、**跨 worktree 不互斥**；SSoT `headRef` 要求解析 sha 等于当前 HEAD 而每个 worktree 有独立 HEAD）；**隔离五条** → 同节 `:248-256`：① Step0 已隔离检测 + `git rev-parse --show-superproject-working-tree` submodule 守卫 ② 建前取用户同意 ③ 原生 `git worktree` 优先（不用平台工具或手工目录绕过）④ `git check-ignore` 强制 ⑤ clean baseline 强制；**清理拥有权判定 + `prune` 自愈** → 同节 `:258-261`（只清理位于 `.worktrees/`／`worktrees/` 下**且由本次工作创建**的 worktree，非自建一律不清理、PR 未落地前不删；元数据残留用 `git worktree prune` 自愈、不做手工 `rm -rf` 元数据）+ 归档侧 `w-model-dev/references/phase-8-acceptance-test.md:274-277`「worktree 收尾归属（S23）」；**不存在自动 `git reset`／自动清理用户文件／绕过 CHECKPOINT 的路径** → 同节边界段 `:263-267`（git-only 纯文档规则、**不得自动 `git reset`**、**不得自动清理用户文件**，另含不新增检测脚本）；证据：`grep -n "worktree 纪律" w-model-dev/references/phase-5-coding.md` → `232:## worktree 纪律（S23）`；`grep -n -e "show-superproject-working-tree" -e "check-ignore" -e "prune" -e "clean baseline" -e "git reset" w-model-dev/references/phase-5-coding.md` → `252`／`255`／`256`／`258`／`261`／`266` 全命中；`grep -n -e "prune" -e "拥有权" w-model-dev/references/phase-8-acceptance-test.md` → `274:...拥有权判定...`／`276:...worktree prune 自愈...`。**M12 的 `pre-commit`（AC-9 相邻交付物，顺带记录）**：`.githooks/pre-commit` 在盘、index 模式 **100755**（`git ls-files -s .githooks/pre-commit` → `100755 1380ea079505b56556f86fafdddd21099f750df0 0 .githooks/pre-commit`）、staged-only、**绝不跑全量 vitest**（慢层仍在 `pre-push`）、缺 node／node_modules／prettier 时降级告知并放行、不阻断提交 |
| AC-10 | M08 的拒绝登记**挂靠既有事实源**（非自由目录），含概念粒度条目、`Prior requests` 回链、入口按概念相似度去重的读取动作与对应门禁；仅 rejected enhancement 入册；RTM 未新增关系或字段。**已达成（P6，2026-09-16，各证据命令均真实跑）**：①**挂靠既有事实源（非自由目录）**——登记落 `w-model-dev/templates/requirement-spec.md` §8（既有节）与 `docs/phase1-requirements/requirement-spec.md` 的项目侧 §8；**零新增目录、零新增 `.w-model/*.json`**；`git diff --diff-filter=A --name-only ad48e469..HEAD` → 恰 **1** 个（`docs/superpowers/plans/2026-09-16-p6-rejection-knowledge-base.md`，即 P6 计划本体）。②**概念粒度条目**——五列表格（`conceptKey` / `拒绝理由` / `Prior requests` / `状态` / `来源`）；一概念一行、`conceptKey` 归一化后唯一（由门禁确定性校验）。③**`Prior requests` 回链**——该列非空（无回链写 `-`，显式空、不得留白）；标识取既有可引用体系（`REQ-xxx` / 阶段轮次 `<轮次>-<序号>` / 请求原文短引），**不发明第二套编号**（`phase-1-requirements.md:184` 登记规则第 3 条）。④**入口按概念相似度去重的读取动作**——`w-model-dev/references/ingestion-chunk.md`「需求入口读取动作（拒绝登记去重，M08）」（`:75`）固定五步（读 §8 全量 → 按**概念相似度（非关键词）**匹配 → surface → **Confirm / Reconsider / Disagree** 三选一 → **判定必须口播**），并明写「**不改变 ingest 的既有确定性分流**、**不是新路由**、也不新增 `/wm` 子命令」（`:92`）。⑤**对应门禁**——既有 `check-artifact-gate.ts --phase=1 --spec-dir` 经 `w-model-dev/scripts/logic/gate-logic.ts` 的 `checkOutOfScopeRegister`（`:852`）对 `checkRequirementSpecStructure` / `checkPhaseSpecStructure` **双入口共用 helper** 新增 `outOfScope` 桶（节存在 / 五列齐 / `conceptKey` 非空且唯一 / `状态` 枚举 / `Prior requests` 非空 / ≥1 数据行；哨兵行豁免）。**证据必须引用 `GATE_JSON.reasons` 中 §8 桶的计数，不得以整体退出码为据**——三态退出码会被 tla/bdd/uat 无关缺件掩盖（本波已实测确认）。本次实跑（自建临时 phase-1 项目、用后 `rm -rf`；`.w-model/rtm.json` 取 `samples/gate/valid-rtm.json`）：**合规模板 §8 逐字** → `GATE_JSON.reasons` 总数 **6**、§8 桶 **0**、`SEC8_ENTRIES=[]`、整体 exitCode=1（仅因无关缺件，如实未宣称 exit 0）；**旧散文 §8**（`- {{…}}`）→ 总数 **7**、§8 桶 **1**（`structure: §8 无固定列表格（旧散文形态「- {{」须表化：conceptKey / 拒绝理由 / Prior requests / 状态 / 来源）`）。⑥**仅 rejected enhancement 入册**——`w-model-dev/references/phase-1-requirements.md:185` 登记规则第 4 条明写「**仅 rejected enhancement 入册**；**bug / 缺陷不入册**；**已实现者拒收**」；`grep -n "拒绝登记规则\|表格形态（门禁强制项）" w-model-dev/references/phase-1-requirements.md` → `:174` / `:188`。⑦**RTM 未新增关系或字段**——`git diff ad48e469..HEAD -- w-model-dev/schemas/rtm.schema.json` **为空**；`ls w-model-dev/schemas/*.schema.json \| wc -l` = **34**（零新增 Schema）。**边界措辞（整分支最终审查裁定，不得省略）**：「对应门禁仅校验**登记结构**（节存在 / 五列齐 / `conceptKey` 非空且唯一 / `状态` 枚举 / `Prior requests` 非空 / ≥1 数据行），**不校验概念相似度**；概念相似度由入口读取动作的**语义匹配**承担。**「门禁通过」≠「去重已发生」**——两者是「结构门禁 + 语义读取」的分工。」**另两条如实说明**：(a) **P6 不含 Schema 变更**——M08 **不在**规格 §5 的 P0 已批准 C 级产物清单内（属 §6 `:215-223` D 级），故本计划**不新增任何 CLI 与 Schema**（实测 `cli` **46** / `schemas` **34** / `references` **43** / `self-test` **352** 均未变；exit-2 **45** = `cli/*.ts` 46 − `self-test.ts`，口径同 `check-samples-coverage.ts` 动态推导，本波未独立跑 docs-consistency；`runLogActionCount` 27 / `prePushCount` 18 / `maxAntiPattern` 48 未变）；**D7 的「D7 确认」前置**由「不触及 RTM/SSoT + M08 在既有命令内实现（规格 §6 `:221`）故 D1–D6 不适用」满足。(b) **AC-11 的判据强化披露**见本表 AC-11 行。 |
| AC-11 | 全部采纳项**不改变**既有 `/wm` 命令的输入/输出/语义；不新增阶段放行路径；不新增 RTM 关系或覆盖率语义（M07 的 Schema 变更单独批准）；不引入新 npm 运行时依赖、外部工具链、**平台/会话 hook**、HTML 产物、CDN 依赖或本地服务；`npm run audit:l0-links` 保持 exit 0。**P1 范围内已满足，P2–P6 落地时须复验**：`git diff 228b1168..HEAD -- package.json` 为空（无新 npm 依赖）；P1 仅新增/修改 Markdown 与一个既有测试 helper（`l0-baseline.ts`），无新增脚本/Schema/hook/HTML/CDN/本地服务；`npm run audit:l0-links` exit 0、`violations: []`（`relativeLinkCount`=671 / `l1OnlyCount`=93 / `templatePlaceholderCount`=36）；`npm run prepush` exit 0。限定：P1 改动面为纯文档 + 计数同步 + 既有 helper，故命令/RTM/Schema/依赖语义未变的判定仅覆盖 P1；P2–P6 若触及 `/wm` 命令、RTM 关系、Schema 或依赖，须以同一组命令复验。 **M07 已达成（独立批准单元，2026-09-15 用户单独批准，裁定 D-2；该单元只新增可选字段与校验/文档义务，不改 `/wm` 命令输入/输出语义、不新增阶段放行路径）**：① `rtm.schema.json` 新增**可选** `testSummary.evidence`（`command` / `exitCode` / `observedAt` + 可选 `rawOutputPath` / `rawOutputSha256`），RTM 实体、关系与覆盖率语义不变、schema 文件计数仍 34；② `check-artifact-gate.ts` 经 `gate-logic.ts` 新增 E1–E4 四规则：E1 配对（`rawOutputPath` ↔ `rawOutputSha256` 要么都有要么都无）、E2 哈希核验（相对项目根解析 / 禁越出根 / 文件须存在 / SHA-256 须相符）、E3 结果一致性含 RED 绑定（`failed>0 ⇒ exitCode≥1`）、E4 存在性 + cutoff（`M07_TEST_EVIDENCE_CUTOFF='2026-09-15T00:00:00Z'`，缺失/不可解析保守不吸收，早于 cutoff 吸收为非阻断 `LEGACY_TEST_EVIDENCE`）；③ 六组 `samples/gate/` 夹具（正例携真实 sha256 产物 / legacy 正例 / E1/E2/E3/E4 各一反例）登记进 `GATE_CASES`（26 条）；④ 文档落点：`rtm-guide.md` 新节 / `data-models.md` 语义段 / `command-reference.md` S 义务 / `templates/rtm.md` 注；⑤ 与 run-log `revertEvidence` 载体不同、不得互替。证据命令（本次收口逐条真实跑）：`npx tsx w-model-dev/scripts/cli/check-artifact-gate.ts <临时项目> --phase=6` 对正样本 `testEvidence {"checked":2,"withEvidence":2,"missing":0,"legacy":0,"e1":0,"e2":0,"e3":0,"e4":0}`、legacy 样本 `"legacy":2`（两条 `LEGACY_TEST_EVIDENCE ... deferred`）、四个反样本各恰好命中一条（E1 `"e1":1` / E2 `"e2":1` 且 reason 报 `SHA-256 不符` / E3 `"e3":1` 且 reason 报 `记录 failed=1 但 evidence.exitCode=0` / E4 `"e4":1`）——样本级隔离项目缺 BDD/cucumber/codegraph/opsx 无关必须件，故整门 exitCode=1，上述为证据子结果；`npm run self-test` → 尾行「总计 340 条用例：340 通过，0 失败」（含六组 gate 用例）；`grep -cE '^\| [a-z]' w-model-dev/scripts/samples/NEGATIVE-COVERAGE.md` = 44；`ls w-model-dev/schemas/*.schema.json \| wc -l` = 34；`npm run audit:l0-links` exit 0（`violations: []`，本次 672/95/36）；`npm run prepush` exit 0（18 项门禁全绿，本次复验）。**P6 范围披露（2026-09-16）**：P6 对 **phase-1 结构门禁**做了**数据要求收紧**——`docs/phase1-requirements/requirement-spec.md` 的 §8 须为合规的五列表格（含 ≥1 数据行或哨兵行），否则阶段 1 结构校验报 `structure: §8 …` 违规（exit 1）。**该收紧不新增命令、不新增放行路径、不改 `/wm` 命令的输入/输出语义**，属**既有门禁的判据强化**（`command-reference.md` 已显式声明，非「门禁未变」）。P6 复验 AC-11：`git diff ad48e469..HEAD -- package.json package-lock.json .githooks/` 为空（零新依赖、零 hook）；无 HTML/CDN/本地服务；`npm run audit:l0-links` exit 0。 |
