...
- 实现：直接调用 `LLMClient` 生成 N 个正交维度，每个含 `id`、`description`、`scoringPrompt`、`weight`（归一化到 1）、`minThreshold`、`levelDescriptors`（5 级）
- 缓存：内存 Map，key = `${type}:${hash(taskDescription)}`
- 失败回退：LLM 失败或解析失败 → 返回硬编码 subCriteria + `rubricFallback=true`；硬编码 subCriteria 的每个维度用 `minThresholdDefault` 填充 `minThreshold`，保证 fallback 路径仍受 DimensionAwareFilter 保护
- 依赖：`LLMClient`、`VerifierConfig.rubric`（注：直接使用 `LLMClient` 生成 JSON，不经 `LLMVerifierEngine`——后者是评分专用、含 logits 期望值逻辑，不适合 JSON 生成）
...
- `tests/verification-framework.test.ts` 扩展：
  - α 已知夹具：完全一致 → α=1；完全反向（ordinal）→ α<0；完全随机分歧 → α≈0
...
3. α 计算在完全一致夹具上 =1、完全反向（ordinal）夹具上 <0、完全随机分歧夹具上 ≈0。注：单模型重复评估场景下"完全反向"几乎不会发生（模型不会对同一产物给出完全反向的多次评分），故该夹具仅用于验证 ordinal α 的数学行为边界。
