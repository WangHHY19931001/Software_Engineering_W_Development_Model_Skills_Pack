...
> **处置回填（2026-09-06，task 9 收口）**：主表「处置建议」列已按战役实现逐条落定并括注实现任务与 commit——6 I + 48 M 全部 fix；唯一例外 F-G4-03 defer（T1-T8 步骤清单均未覆盖，见该行标注）；F-G6-03 维持「已裁定不改」。说明级 64 条：13 条 fix（S2/S3/S4/S18/S19/S20/S22/S25/S46/S49/S56/S57/S64）、51 条 defer。全量回归与 prepush 证据见验收记录 `docs/changes/2026-09-01-42.2.1-audit-remediation-acceptance.md`「核查发现修复（audit-fixes）验证记录（2026-09-06）」节。
...
| F-G4-03 | M | G4 | `schemas/run-log.schema.json:209`；`logic/run-log-logic.ts:884-888` | gateLogPath「必填」描述无任何强制点（官方样例 phase5-valid.jsonl gate 行无此字段仍 exit 0） | 补条件 required 或改描述 | g4-schema-logic/a1-report.md#发现7 | defer（T1-T8 步骤清单均未覆盖此项：T4 brief 仅含 F-G4-01/02；现状描述已含「gate/tla-gate/graph-gate 类动作必填」限定与 R6 消费口径，条件 required 强制点仍未补，留档后续批次） |
