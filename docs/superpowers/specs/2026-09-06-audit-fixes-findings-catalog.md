...
| F-G4-03 | M | G4 | `schemas/run-log.schema.json:209`；`logic/run-log-logic.ts:884-888` | gateLogPath「必填」描述无任何强制点（官方样例 phase5-valid.jsonl gate 行无此字段仍 exit 0） | 补条件 required 或改描述 | g4-schema-logic/a1-report.md#发现7 | fix（sha7 见随后的 docs 回填提交，描述对齐实现） |
