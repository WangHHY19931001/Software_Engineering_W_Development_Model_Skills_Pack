...
**步骤 0 · 前置检查（R1 处置方案，未通过不得进入 touchpoint 1）**

- **命令**：`npx tsc -p config/tsconfig.json`（与 SSoT §10H.5 V1 及新增第 17 项完全同源）
- **通过判据**：退出码 0（README:21「0 错误」手测值验证成立）→ 直接进入 touchpoint 1
- **未通过路径**（退出码非 0）：
  1. **错误清点**：记录错误总数、涉及文件数、错误码分布（如 TS2322/TS2345 等），形成存量错误清单
  2. **分类分诊**：区分「真实类型缺陷」与「环境性/配置性问题」（缺 @types 声明、tsconfig include 范围偏差）
  3. **汇报门槛（强制）**：向用户汇报清单与工作量（错误数、文件、预计改动面），**获批后才执行修复**——不得擅自扩大改动面
  4. **修复原则**：最小修复，只消类型错误、禁止顺手重构/格式化；每修一批重跑 tsc 确认错误数单调递减至 0
  5. **修复后回归**：tsc 退出码 0 + `npx vitest run` 全绿（确认类型修复未改变运行行为）→ 进入 touchpoint 1；存量错误修复过程记入 CHANGELOG 41.19.0 批2 条目
- **若存量错误量大到需独立轮次**：暂停批2，向用户提议拆分为独立修复轮（本计划只保留门禁接入部分），由用户决策
...
收尾验证：`npm run self-test` + `npx vitest run` + `npm run typecheck`（touchpoint 1 落地后即以 script 形式复验步骤 0 的通过态）。
...
| 风险 | 处置 |
|---|---|
| R1 tsc 存量错误（README「0 错误」为 2026-08-18 手测值，可能过期） | 批2 步骤 0 前置检查：实跑 tsc，非 0 时按「清点→分诊→汇报获批→最小修复→回归」流程处置（详见 §4 批2 步骤 0），量大则拆独立修复轮由用户决策 |
