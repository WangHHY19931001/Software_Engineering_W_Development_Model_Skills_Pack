...
| `STRUCTURE_INVALID` | 通过解析但不符合 JSON Schema | 对照 `w-model-dev/schemas/` 下对应 schema（由 [`schema-loader.ts`](../w-model-dev/scripts/infrastructure/schema-loader.ts) 加载；34 份清单见 [`data-models.md`](../w-model-dev/references/data-models.md)） |
...
> 修改 `*-logic.ts` 校验逻辑后必须跑 `npm run self-test`（358 条样本回归基线）与 `npm run prepush`，并同步样本（见 [`__tests__/README.md`](../w-model-dev/scripts/__tests__/README.md) coverage 矩阵）。
