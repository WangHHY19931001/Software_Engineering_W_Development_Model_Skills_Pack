...
**L0 复用分级**：references 分「可单独拷贝」（方法论自包含：root-cause-locator / iceberg-sweep-guide / agent-personas+subagent / conventions / estimation-guide / context-management-guide / coding-quality / activation-guide / toolbox）——其中 `agent-personas.md` + `subagent/` 的 L0 副本须保留每份人格 frontmatter 的 `capabilities` / `inputs` / `outputs` / `boundaries` 四字段（§6.4.5 契约，L0 拷贝不得剥离）——与「不可单独拷贝」（依赖编排/状态/门禁：phase-N-* / subagent-delegation / signature-chain-guide / rtm-guide / graph-guide / hard-constraints）；权威清单与版本对齐义务见 docs/INSTALL.md「子能力单独复用」节。
...
**R-persona 两键选择矩阵（权威定义，实施见 agent-personas.md「Persona 矩阵」§2）**：

- **第一键 = `rootCause.category`**（R 自述，schema 枚举，必选）：7 个类别各有一行候选 persona 集合。
- **第二键 = 风险域信号**（安全 / 性能 / AI-LLM，从 V/G 产出特征读出，不由 R 自述）：命中即**叠加**到第一键行，不是替换；两键并集仍受 §4 数量上限（R-persona 上限 5）约束。
- **第一键分歧**：第二键信号暗示的类别与 R 自述不一致时，R 必须改判 `category` 或在 `rootCause.evidence` 显式记录分歧与理由（静默保留不一致视为选择漂移）。
- **一致性强制**：`root-cause-logic.ts` 的 `R_PERSONA_MATRIX` / `R_PERSONA_SIGNAL_MATRIX` 是 R11 判据源；`check-docs-consistency.ts` 的 `rootcause-persona-matrix` 检查独立解析两边源码与表格逐行对账，并断言矩阵引用的每个 persona 都存在于 `subagent/<name>.md`。

**persona 能力声明四字段（门禁强制）**：每个 `subagent/*.md` 的 YAML frontmatter 必须含 `capabilities` / `inputs` / `outputs` / `boundaries` 四项单行非空字段（`check-docs-consistency.ts` 的 `persona-capability-declarations` 检查强制）。`boundaries`（适用 / 换人）是 R-lead / V-lead 分派时刻的换人判据——缺它则「按需加载」退化为「照抄矩阵」。
...
| 污染源定位（S24，按需工具） | **落点**：`w-model-dev/scripts/cli/check-pollution.ts`（判据纯函数：`w-model-dev/scripts/logic/pollution-logic.ts`）。**输入**：`--project=<dir>`（仅等号形态；缺省 cwd）。**检查对象**：`.w-model/` 残留（`*.lock` 陈旧锁目录/文件）、`*.lock` 锁文件、`coverage/` 残留（含 `coverage/.tmp`）、vitest 语义污染形态（`--outputFile` JSON 残留等，项目根深度 1 名称白名单）；扫描剪除 `node_modules/`、`.git/`。**语义声明**：逐文件定位「存在测试失败痕迹但产物却被判通过」的污染形态（「吞掉测试失败只看产物」），供污染源二分定位。**退出码**：0=干净（stdout 单行 `POLLUTION_JSON` 摘要）/ 1=发现污染源（逐项列出 + `POLLUTION_JSON`）/ 2=输入错误（未知/重复 flag、`--project` 不存在 → `ARG_INVALID`/`FILE_NOT_FOUND`，零副作用）。**按需工具声明**：只作按需工具（规格 :187/:305），**不得当门禁**——不进 pre-push 19 项（`prePushCount: 19` 不变）、不进任何阶段门；exit 1 仅供人工二分定位参考 |
...
> **RTM 覆盖率口径（2026-09-19）**：RTM 覆盖率按**追溯字段**重算——`w-model-dev/scripts/logic/gate-logic.ts` 的 `computeRtmTraceCoverage(rows, phase)` 是单点事实源（REQ 行按 `PHASE_TRACE_FIELDS[phase]`；NFR/CON 行按 `description`/`designDoc`，`phase>=5` 另加 `codeModule`，即 `phase<5` 变体仅 `description`/`designDoc`），`/wm status` 的 `rtmCoverage` 与 `check-artifact-gate.ts` 的 `coveragePercent` 共用该函数、**在同一 `phase` 下**两者数值必然一致（`wm-status` 取 `project.status` 对应阶段，聚合门取 `--phase`、缺省 8；阶段输入不同则字段集不同、数值不保证相同）；`coverageStatus` 仅为展示字段，不参与计算。
...
5. **层次一致性校验**（设计文档 §3.1 步骤 3）：`checkHierarchy()` 校验 parent/child 双向（A.parent=B ⇒ B.children 含 A；A.children 含 C ⇒ C.parent=A）+ sibling 双向（A.siblings 含 B ⇒ B.siblings 含 A）+ 有且仅有一个 L1 根（`parent=null ∧ level=L1`）+ 层级单调（子规格 `level` = 父规格 `level` + 1）。违反 → `hierarchyViolations++`。
   **措辞约定（2026-09-19）**：`children`/`parent`/`siblings` 指向的路径若存在于 manifest 但被当前 `--phase` 过滤掉（`spec.phase > phase`），违反消息须报「属后续阶段（phase=N；当前校验 phase=M 不包含它），不算 manifest 缺失」，不得报成「不在 manifest 中」——后者会把「校验范围收窄」误报为「manifest 未登记」。判定结果不变（仍计入 `hierarchyViolations` 仍拦截）；查不到声明 phase 时退化为「属后续阶段（当前校验 phase=M 不包含它）」。
...
> 返工循环中 R 子代理产出的 `RootCauseReport` 校验门禁，与 §10.5 工件质量门（阶段 8 终检）/ §10.7 图谱门禁 / §10.8 TLA+ 行为门禁互补：前三者校验阶段产物，本节校验返工路径的根因报告。权威定义见 [根因定位者设计 spec](./superpowers/specs/2026-07-24-root-cause-locator-and-fixer-roles-design.md) §4 RootCauseReport Schema 与 R1-R11 校验规则。
>
...
- **回滚**：记录受控 patch 并 `git apply -R` 反向应用；apply 拒绝分支以 **pre-change 快照 `git status` 回读**证明还原（干净工作树下等价于 `git diff --exit-code` 为 0）；P3 `--guard` 在最终证明失败时以 `git diff --exit-code` 为 0 证明还原；无法回滚即显式失败（不静默）。
