...
- **「流程反模式」（[`anti-patterns.md`](../w-model-dev/references/anti-patterns.md) 17 条）** 是流程破坏，命中即回退；与门禁脚本退出码精确对应。
...
```json
{
  ...
  "specs": [{
    ...
    "parent": null,
    "siblings": [],
    "children": ["tla/L2-auth-subsystem.tla"],
...
9. **TLC 模型检查**（仅 SANY 通过且未 `--skip-tlc` 时；cwd 置为 `.tla` 所在目录）：`java -cp <jarPath> tlc2.TLC -nowarning -cleanup -config <spec>.cfg <moduleName>`，捕获 stdout。
   ...
   - `<moduleName>` 为 `.tla` 文件名去后缀（如 `L1-blog-system.tla` → `L1-blog-system`），**非** `.tla` 路径。
...
| 5–8 | `tla-manifest.json` 冻结只读；TLA+ 不变式作为测试 oracle（编码与测试须与不变式一致） | 只读 |
...
| 4A 核心操作行为与失败模式 | 6 条核心操作行为 + 10 条失败模式（F1~F10）+ 6 条运维失败模式（O1~O6）+ 与约束/反例的关系 | `w-model-dev/SKILL.md`「核心操作行为」节 + `w-model-dev/references/anti-patterns.md`「失败模式清单」节（F1~F10）+「运维失败模式清单」节（O1~O6） | 完整（F1~F10 吸收自 addyosmani/agent-skills；O1~O6 吸收自 cobusgreyling/loop-engineering `docs/failure-modes.md`，适配 W 模型语境） |
