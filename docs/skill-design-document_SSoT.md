...
**关键决策**：① 版本门禁 fail-loud（任一来源不可解析即违规，不静默放行）；② 行为门禁开关放编排层而非脚本参数（避免重蹈 `--skip-tlc` 削弱门禁覆辙）；③ 约束重排不触碰归档（docs/superpowers、docs/changes、eval、CHANGELOG-archive 保留历史编号）；④ `w-model-dev/package.json` 维持不声明 version（C7 决策不变，评审建议不采纳）。
