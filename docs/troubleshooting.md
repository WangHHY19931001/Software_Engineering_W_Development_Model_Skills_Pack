...
> **状态：已处置（2026-09-12）。** `config/vitest.config.ts` 将测试拆为两个 project：`cli-serial`（真实启动子进程的测试文件，`fileParallelism: false`，互不重叠）与 `unit-parallel`（其余纯逻辑文件，保持并行）。本节保留现象、根因与排除过程，供后续维护者理解**为何不能让子进程类文件彼此并行**。（清单条目数随新增子进程类测试增长——2026-09-18 收口实测 `SUBPROCESS_TEST_FILES` 为 **40** 个；「30」是拆分当时的条目数，不是设计常量。）
