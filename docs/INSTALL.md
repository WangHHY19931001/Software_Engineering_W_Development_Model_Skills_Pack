...
│   ├── cli/            # CLI 入口层（25 个 check-*.ts 门禁入口 + 9 个工具 CLI：security-scan / wm-status / metrics-report / ensure-codegraph-opsx / wm-write / doctor / plan-chunks / wm-export-evidence / wm-verify-evidence-source；exit-2 脚本口径 = 25 check + 9 工具 CLI = 34，self-test.ts 单列（回归基线，非 exit-2）；IO 抽离，传纯数据给 logic 层）
...
- **devDep（测试）**：`vitest` + `@vitest/coverage-v8`（`w-model-dev/scripts/__tests__/` 单元测试，55 个 test 文件 / 929 条）
