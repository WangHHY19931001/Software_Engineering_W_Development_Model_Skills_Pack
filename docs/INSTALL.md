...
│   ├── cli/            # CLI 入口层（26 个 check-*.ts 门禁入口 + 6 个 code-health 门禁 CLI：code-health-phase1 / code-health-gap / code-health-tests / code-health-duplicates / code-health-ledger / code-health-apply + 10 个工具 CLI：security-scan / wm-status / metrics-report / ensure-codegraph-opsx / wm-write / doctor / plan-chunks / wm-export-evidence / wm-verify-evidence-source / platform-deps-install；exit-2 脚本口径 = 42，self-test.ts 单列（回归基线，非 exit-2）；IO 抽离，传纯数据给 logic 层）
│   ├── logic/          # 纯函数校验逻辑（37 个 .ts：35 个 *-logic.ts + code-health-contract.ts / code-health-phase-boundaries.ts）
...
| 回归基线脚本（322 条样本）                                                                                        | [../w-model-dev/scripts/cli/self-test.ts](../w-model-dev/scripts/cli/self-test.ts)                                                                                                                          |
