...
│   ├── cli/            # CLI 入口层（26 个 check-*.ts 门禁入口 + 7 个 code-health 门禁 CLI：code-health-phase1 / code-health-gap / code-health-tests / code-health-duplicates / code-health-ledger / code-health-apply / code-health-archive + 10 个工具 CLI：security-scan / wm-status / metrics-report / ensure-codegraph-opsx / wm-write / doctor / plan-chunks / wm-export-evidence / wm-verify-evidence-source / platform-deps-install；exit-2 脚本口径 = 43，self-test.ts 单列（回归基线，非 exit-2）；IO 抽离，传纯数据给 logic 层）
...
**推送触发范围（真实 push 语义）**：git push 时 hook 读取 stdin 的 ref 行（每行 `<local ref> <local sha> <remote ref> <remote sha>` 四字段，支持多 ref 逐行聚合）判定变更范围：local sha 全零 = 删除远端 ref（该行无本地内容可检，跳过）；remote sha 全零 = 全新分支，经 `git merge-base --fork-point`（退化为普通 merge-base）建立可证明基线，基线为空或退化为推送尖本身时降级经 remote-tracking 排除集枚举证明——remote 名经白名单与 `git remote get-url` 验证后执行 `git log -m --name-only --pretty=format: <local_sha> --not --remotes=<remote>`，`-m` 确保合并提交按父逐个列出避免空 diff 漏检；三级全部失败才 → fail-closed；常规更新按 `remote_sha..local_sha` 做 `git diff --name-only`。任一 ref 行解析失败（空行/字段不齐/sha 非 40 位十六进制）→ **fail-closed 运行全部门禁**；全部行均为删除（delete-only）→ 放行；stdin 为空（手动执行等非 git push 场景）→ 回退 `HEAD@{push}`（不可用回退 `origin/HEAD`）范围判断，回退失败同样 fail-closed。路径命中才跑门禁：`w-model-dev/**`、根级 `README.md`/`AGENTS.md`/`CONTRIBUTING.md`/`.gitignore`/`.eslintsecurity-baseline.json`/`package.json`/`package-lock.json`、`config/**`、`scripts/**`、`.githooks/**` 与 `docs/*.md`（bash case 模式 `*` 跨 `/`，含 `docs/` 任意层级）。若推送后未见门禁输出且变更确实命中上述路径，用 `bash .githooks/pre-push --force` 或 `npm run prepush` 补跑。
...
| 代码健康治理（`/wm code-health` Phase 1–4）                                                                       | [../w-model-dev/references/code-health-governance.md](../w-model-dev/references/code-health-governance.md)                                                                                                  |
| 负面知识库（48 条反模式 + 教训）                                                                                  | [../w-model-dev/references/hard-constraints.md](../w-model-dev/references/hard-constraints.md)                                                                                                              |
...
| 回归基线脚本（358 条样本）                                                                                        | [../w-model-dev/scripts/cli/self-test.ts](../w-model-dev/scripts/cli/self-test.ts)                                                                                                                          |
...
- **R 根因子代理**：V/G 不通过后，R-lead 按两键矩阵（`rootCause.category` + 风险域信号，叠加取并集）选用多角度做根因定位，多角度报告的 persona 选择由 `check-rootcause-report.ts` 的 R11 校验（矩阵内 + 与第一键行有交集）（详见 [`references/root-cause-locator.md`](../w-model-dev/references/root-cause-locator.md)）
