...
1. 先按 [`README.md`](../README.md) 的「验证仓库」入口执行仓库验证命令（完整命令序列以该入口的命令块为唯一权威，本节不复述）。这一步验证仓库脚本健康，不会安装 Agent Skill。
...
- **如实状态**（硬约束 #7）：未完成、未评审或未确认的阶段不得标为完成。
...
- **估算覆盖率放行**：违反 硬约束 #4，与 Greenfield 反模式相同——棕地不是降低标准的理由。
