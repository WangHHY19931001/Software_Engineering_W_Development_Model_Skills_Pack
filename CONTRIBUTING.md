...
在 `git push` 时自动跑 19 项检查；任一退出码不符预期即中止推送：

**迭代车道 vs 验收车道（规则）**：本地迭代可以用快速车道 `npm run test:affected`——只跑「本次改动映射到的测试文件」，用法与边界见 [README 的「验证仓库」](./README.md#验证仓库)。但**任务收口、交付与合入前的验收必须跑全量**：`npm run prepush`（19 项）或至少 `npm test`（全量 vitest）。快速车道按文件名映射选测试，天然覆盖不到跨文件的注册表类不变量（「每个 exit-2 门禁都登记了负向案例」「活体文档与脚本计数一致」「samples 覆盖矩阵闭环」等），因此**不得**把它的通过结果当作验收依据；触及 `config/**`、`.githooks/**`、`schemas/**`、`references/**`、`samples/**`、`docs/**`、`eval/**`、根 `scripts/**` 与根活体文档时，该脚本会自动退回全量。
