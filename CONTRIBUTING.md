...
```bash
# 3.1 单元测试（vitest，55 个 test 文件 / 942 条，含各 *-logic.ts 纯逻辑与 CLI 集成测试）
...
| 12 | `npx vitest run --coverage --config config/vitest.config.ts`（单元测试全量 + 覆盖率阈值门禁：stmts 75 / branch 65 / funcs 85 / lines 75，阈值不达标 vitest exit 1；55 files / 942 tests） | 0 |
...
│   ├── __tests__/                 # vitest 单元测试（55 个 .test.ts / 942 条 + README.md coverage 矩阵）
