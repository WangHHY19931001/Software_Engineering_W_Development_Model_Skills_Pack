 ...
 *
 * package.json 为唯一版本源，本脚本一处改版、六文件同步：
 *   - package.json                    "version"
 *   - w-model-dev/skill-metadata.json "version"
 ...
 *
 * 改版后用 `npm run check:docs-consistency` 自验证（version-consistency 检查比对六处）。
 * net effect：从「每次发版手改 6 文件 × 7 处」降为「一条命令 + 填 CHANGELOG 正文」。
 *
 * 用法：
 *   node scripts/version-bump.cjs <new-version>                     # 升级并同步六处
...
function run() {
  ...
  if (dryRun) {
    ...
    for (const t of targets) {
      ...
      log(`  - ${t.label}（${t.file}）：${replaced ? '将替换' : '✗ 未找到版本点'}`);
    }
  ...
  log('完成。下一步：`npm run check:docs-consistency` 自验证六处版本一致 + 填写 CHANGELOG 条目标题/正文。');
  return 0;
}
