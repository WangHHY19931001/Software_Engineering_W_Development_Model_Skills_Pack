...
export class RubricGenerator {
  ...
  async generate(type: RubricType, taskDescription: string): Promise<GenerateResult> {
    ...
    try {
      ...
      // 回退硬编码
      const result: GenerateResult = { subCriteria: hardcodedRubric(type, this.config.minThresholdDefault), fallback: true };
      if (this.config.cache) this.cache.set(key, result);
      return result;
    }
  }
...
function hardcodedRubric(type: RubricType, minThresholdDefault: number): SubCriterion[] {
  switch (type) {
    case 'requirement':
      return [
