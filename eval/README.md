...
- 技能当前版本 35.0.0（第 35 轮收尾）后**尚未补跑评估**——TSV 中最后一条 dry_run 记录停留在 2026-07-20（darwin-02-11，90.5 分）；第 32/33/34/35 轮改动（错误结构全量归一化、run-log R6 契约迁移、5 批优化收尾、目录约定/格式统一/覆盖率校验架构升级、8 阶段调测修复入库等）待外部 darwin-skill 补跑验证，补跑流程见上文。
