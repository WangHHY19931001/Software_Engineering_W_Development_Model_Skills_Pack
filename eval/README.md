...
| # | 行动项 | 前置条件 | 负责人（占位） |
|---|---|---|---|
| 1 | **e2e 重跑**：w-model-dev-demo 8 阶段生产规模从零重建（非 self-as-verifier 模式），验证 41.17.0 全流程 | 41.17.0 合入 + 环境就绪（`npm run doctor` 全绿） | 待指派 |
| 2 | **darwin-skill 盲评补跑**：按上方「待评估版本」表，用 `w-model-dev-test-prompts.json`（25 条）驱动对新版本技能评分 | 41.17.0 合入 + darwin-skill 外部工具可用 | 待指派 |

> 本轮（41.17.0）不伪造评估证据，如实记录缺口现状；上述行动项在版本合入后按序执行，结果回填本状态节与 TSV。
