...
## Descriptor-relative archive race closure (2026-08-28)

This round supersedes the preceding uncommitted staging-based and 68-test follow-up evidence. The implementation under review is the direct descriptor-relative extractor in this worktree, based on `4d9f916406bf5bc1b3d158f36ca27ce6007e6b36`. The pre-amend implementation snapshot commit is `46f0b1a11bc977baa490024d80095408968334fe`; the delivery remains the single commit named `fix(security): close archive race boundary`. Its authoritative post-amend SHA is recorded in the completion handoff because a Git commit cannot include its own hash in its content without changing that hash.

### Actual implementation

- Archive parsing and canonical preflight complete before capability checks or filesystem work. The preflight rejects absolute or drive-qualified paths, traversal, dot/empty segments, NUL bytes, symlink/hardlink entries, any `linkname`, duplicate canonical paths, and file-ancestor/descendant conflicts. The conflict check now indexes every canonical path and examines every ancestor, so a lexically intervening entry cannot hide a file/descendant conflict.
- POSIX extraction writes directly beneath the caller-provided, pre-existing extraction root. Root and parent directories are opened with `O_DIRECTORY | O_NOFOLLOW`; child directories and files are addressed only through `/proc/self/fd/<parent-fd>/<name>` or Darwin `/dev/fd/<parent-fd>/<name>`. Files use `O_CREAT | O_EXCL | O_NOFOLLOW`.
- Every created directory is identity-checked and registered as owned before its child handle is opened. Every created file is identity-checked and registered before write, chmod, or close. Directory modes are applied through verified handles only after all entries exist.
- Failure rollback walks owned entries in reverse order. It reopens the root and parent chain, checks parent and child `dev`/`ino` identities and object type, rejects links or replacements, and only then performs descriptor-child `unlink` or `rmdir`.
- Windows and platforms lacking `O_DIRECTORY`, nonzero `O_NOFOLLOW`, or an accessible descriptor root fail closed after archive-only preflight and before any extraction handle or write operation. There is no `mkdtemp`, `copyFile`, `rename`, staging-transfer, or path-based extraction fallback in `platform-deps-tar.ts`.
- PAX UTF-8 and `x`/`g` records, PAX `linkpath`, UStar/GNU long names and links, tar modes, duplicate preflight, and link rejection remain covered. No file-level ESLint suppression or security-baseline change was introduced.

### TDD and verification evidence

| Command or checkpoint                                                                                                                                                                                                                                                                    | Actual result                                                                                                                                                                                                                                                       |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Initial required `npx vitest run --config config/vitest.config.ts w-model-dev/scripts/__tests__/platform-deps-install.test.ts`                                                                                                                                                           | Passed: 1 file / 72 tests on the current `win32` runner. There were no initial failures; the prior 68/68 result is not used as current evidence.                                                                                                                    |
| Added non-adjacent file-ancestor regression before the production fix                                                                                                                                                                                                                    | Failed as intended: 1 failed / 73 passed. The implementation reached the platform capability refusal instead of rejecting the hidden file/descendant conflict during archive-only preflight.                                                                        |
| Same test file after the minimal canonical-ancestor fix and final static/platform seam additions                                                                                                                                                                                         | Passed: 1 file / 74 tests.                                                                                                                                                                                                                                          |
| Directed related run: `platform-deps-install`, `platform-deps-repair-core`, `evidence-export-logic`, `evidence-provenance-logic`, and `run-sync`                                                                                                                                         | Passed: 5 files / 175 tests (74 + 27 + 38 + 18 + 18).                                                                                                                                                                                                               |
| `npm run typecheck`                                                                                                                                                                                                                                                                      | Passed: `tsc -p config/tsconfig.json`, exit 0.                                                                                                                                                                                                                      |
| `npx prettier --config config/prettier.config.cjs --write` and `--check` limited to `.superpowers/sdd/w-model-reliability-optimization/task-final-security-report.md`, `w-model-dev/scripts/lib/platform-deps-tar.ts`, and `w-model-dev/scripts/__tests__/platform-deps-install.test.ts` | Passed: only the three delivery files were written; the subsequent check reported that all matched files use Prettier style.                                                                                                                                        |
| `git diff --check`                                                                                                                                                                                                                                                                       | Passed: exit 0. Git emitted only the expected LF-to-CRLF working-copy warnings for the three delivery files.                                                                                                                                                        |
| `npm run lint:security`                                                                                                                                                                                                                                                                  | The first finalization run failed with 13 new non-literal-filename findings in the new descriptor/root calls. After adding statement-local boundary rationales, the rerun passed with baseline 410, hits 410, and new findings 0. The baseline was not regenerated. |
| Sequential `npm run prepush`                                                                                                                                                                                                                                                             | Passed: all 17 pre-push gates, including full Vitest plus coverage, npm audit, security scan, docs/samples consistency, Prettier, and typecheck.                                                                                                                    |
| Platform evidence                                                                                                                                                                                                                                                                        | `node -p "process.platform"` reports `win32`. Windows refusal is executed with write/handle injection guards. POSIX operation ordering and flags are verified by a static source seam; native POSIX descriptor syscalls cannot be executed on this Windows runner.  |

### Diagnosed tooling failures

- The shell-level `apply_patch` command was unavailable and exited 127 before changing any file. The same test-only edit was applied with exact-string file editing.
- A report-diff display helper using `/dev/stdin` failed because Git Bash on this Windows host mapped it to inaccessible `/proc/self/fd/0`. It was replaced with native `git diff --cached`, `git diff-tree`, and status checks; delivery validation was unaffected.
- The first exact `git add` returned exit 1 because `.superpowers/sdd/.gitignore` ignores the report directory, although all three already-tracked delivery files were updated in the index. The staged names and index entries were inspected, and the later report-only update used `git add -u`; no ignored runtime output was staged.

### Residual limits

- Secure extraction is intentionally unavailable on Windows and on POSIX systems without the required directory/no-follow flags or descriptor filesystem. This is an availability tradeoff, not a best-effort security claim.
- `extractArchive()` requires the caller-selected root to exist and be an identity-usable real directory. Current installer callers satisfy this by passing an already-created private installation staging root.
- Native POSIX syscall behavior still requires a POSIX runner for end-to-end execution. The current Windows run proves fail-closed behavior and uses static seams for POSIX-only source invariants; it does not claim native POSIX runtime coverage.
- CodeGraph could not inspect this worktree because no `.codegraph/` index exists at or above it. The modification scope and callers were inspected with repository source search instead; no ignored `.w-model`, `coverage`, or `.zcode` artifact is part of the delivery.

### Final command results

The complete three-file delivery set passed targeted formatting, `git diff --check`, security lint with no new findings, and the sequential 17-gate pre-push run. The commit is intentionally limited to this report, `platform-deps-tar.ts`, and `platform-deps-install.test.ts`; ignored `.w-model`, `coverage`, and `.zcode` output is excluded.
