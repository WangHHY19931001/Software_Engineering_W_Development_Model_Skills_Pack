...
阶段 1–4 额外执行 TLA+ 行为门禁（与图谱门禁正交叠加）：`check-tla-model.ts "<tla-manifest.json>" --graph=<graph.json> [--phase=1|2|3|4]`（`--graph` 在 phase>=2 时强制；`--skip-tlc` 已移除不得跳过 TLC）。**阶段 4 TLA+ 零违反 + 图谱零违反才放行进编码**（约束 #13）。阶段 1-8 BDD 行为门禁：`check-bdd-model.ts --phase=N`（D1-D8）；项目阶段门按成熟度要求真实行为证据：phase 1-4 传精确裸 flag `--require-tla-equivalence --tla-manifest=<path>`，phase 5-8 传精确裸 flag `--require-cucumber-report --cucumber-report=<path>`。缺证据是 exit 1 violation；required Cucumber 报告须为合法 elements 数组，且至少一个非空 `name` 的 scenario 含 `status="passed"` step；failed 只诊断失败，skipped/pending/undefined/未知 status 或匿名 element 都不算执行并为 D5 violation。任何 `=true`、重复、拼写近似或未知 `--*`，以及 flag 用于不对应 phase，均为 exit 2；未传 flag 才保留兼容跳过诊断。

项目 Artifact Gate 的 phase 1-4 还必须执行独立 `check-tla-bdd-sync.ts` pair sync，但仅在 TLA/BDD manifest 均通过真实 schema/资产校验且 TLA spec↔BDD feature 配对双向完整覆盖时逐 pair 调用；D4 required equivalence 与独立 pair sync 是两条不可互替的证据路径。缺失/畸形资产由各自 evidence gate fail-closed，不得静默 skip；phase 5-8 不执行 TLA↔BDD 文件同步，使用 required Cucumber 证据。具体契约见 SSoT §10.5.1。**本地 pre-push 仅直接运行 BDD fixture 回归，不传 project-only required flags，也不直接运行 TLA、TLA↔BDD 同步或项目工件阶段门。**
