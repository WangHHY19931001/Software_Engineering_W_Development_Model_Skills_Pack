...
6. **分派 S 子代理产出**（O → S）：分派产出子代理生成开发产物 + 同步测试设计 + 更新 RTM 实体；S 返回 `{产物路径, RTM diff, selfCheck}`。**编排者不得直接产出**。
7. **分派 V 子代理评审**（O → V）：分派评审子代理按 `targetKind` 路由 Persona，产出 `VerifierOutput` JSON。**编排者不得自评**。
8. **分派 G 子代理门禁**（O → G）：分派门禁子代理跑 `check-verifier-output.ts`，返回 `{exitCode, qualityLevel, passed, reworkHints}`。编排者**可同步跑一次只读脚本看退出码**用于展示，但 G 子代理的回填不可省略。
9. **验证与暂停**（O）：若 G 返回 `exitCode=1` 或 `qualityLevel ∈ {C,D}` → 分派 S 返工（带 `reworkHints`），重走 6→7→8；若通过 → 🔴 CHECKPOINT 等待用户决定。
...
| 命令 | 路由 | 关键前置/行为 | 子代理分派 |
|---|---|---|---|
| `/wm analyze <需求>` | 阶段 1 | 首次初始化并同步验收测试设计 | O 路由 → S 产出 → V 评审 → G 门禁 |
| `/wm design type=<架构\|概要\|详细>` | 阶段 2/3/4 | 必须存在上一阶段已放行产物 | O 路由 → S 产出 → V 评审 → G 门禁 |
