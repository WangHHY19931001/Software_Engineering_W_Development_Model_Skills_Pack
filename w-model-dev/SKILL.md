...
每命令的输入、输出、失败动作见 [references/command-reference.md](references/command-reference.md)。门禁脚本 43 个 .ts，登记总览见 subagent-delegation.md「dispatch-matrix」节。
...
- **资源计数**：`references/`（42 个 .md）、`schemas/`（34 份 JSON Schema draft-07，含 change-scope / codegraph-query 与 evidence-manifest / evidence-provenance）、门禁脚本 43 个 .ts。
