...
describe('runDocConsistencyChecks', () => {
  ...
  it('coverage JSON 的 55/993 元数据与活体文档同步 → CLI 消费 JSON 注入计数', async () => {
    const input = baseInput();
    ...
    await withDocsConsistencyFixture(async (fixtureRoot) => {
      const coverage = await writeVitestCount(fixtureRoot, 993);
      expect([(coverage.testResults as unknown[]).length, coverage.numTotalTests]).toEqual([55, 993]);
      ...
      for (const doc of docsWithLiveCount) {
        ...
        expect(docContent).toContain('993');
      }
      ...
      expect(passing.stdout).toContain('vitest 用例  : 993');
      ...
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- mkdtemp-controlled fixture path
      await fs.writeFile(readme, content.replace('55 files / 993 tests', '55 files / 928 tests'), 'utf-8');
      const stale = runDocsConsistencyCli(fixtureRoot);
      expect(stale.code).toBe(1);
      expect(stale.stdout).toContain('vitest 用例  : 993');
  ...
  it('双环境 stable probe map 对单字段 args drift 与 cwd drift 分别失败', async () => {
    await withDocsConsistencyFixture(async (fixtureRoot) => {
      await writeVitestCount(fixtureRoot, 993);
...
async function withDocsConsistencyFixture(assertResult: (fixtureRoot: string) => Promise<void>): Promise<void> {
  ...
  try {
    ...
    // 复制的活体文档保留当前 HEAD 实测的 55/993 计数；各测试仅在需要时注入单独的旧值负例。
