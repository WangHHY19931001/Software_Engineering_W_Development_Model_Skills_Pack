...
const identity2FixturePath = path.join(
  here,
  "fixtures",
  "run-log-identity2-raw.jsonl",
);
...
describe("run-log R1 扩展：rootcause/fix 动作字段", () => {
  it("rootcause 动作缺 reportId 时失败", async () => {
    ...
    expect(
      result.violations.some((r) => /R1.*rootcause.*reportId/.test(r)),
    ).toBe(true);
  });
  ...
  it("rootcause 动作缺 rootCauseCategory 时失败", async () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.violations.some((r) =>
        /R1.*rootcause.*rootCauseCategory|\[schema\].*rootCauseCategory/.test(
          r,
        ),
      ),
    ).toBe(true);
  ...
  it("fix 动作缺 basedOnReport 时失败", async () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.diagnostics?.some((r) => /LEGACY_UNSCOPED.*basedOnReport/.test(r)),
    ).toBe(true);
  });
...
describe("run-log R3 扩展：R + S-fix 一一对应 + V 复审", () => {
  it("有 R 但缺 S-fix 时失败", async () => {
    ...
    expect(
      result.violations.some((r) =>
        /R3.*rootcause.*fix.*一一对应|basedOnReport.*缺失/.test(r),
      ),
    ).toBe(true);
  });
  ...
  it("有 R 但缺 V 复审 rootcause 时失败", async () => {
    ...
    expect(
      result.violations.some((r) =>
        /R3.*V 复审 rootcause.*≠.*R 记录数/.test(r),
      ),
    ).toBe(true);
  });
});
...
describe("run-log R7 扩展：返工路径时序", () => {
  it("有 R 但缺 S-fix 时 R7 时序校验也失败", async () => {
    const lines = await loadJsonl("rootcause-missing-fix.jsonl");
    const result = checkRunLog(lines);
    expect(result.violations.some((r) => /R7.*rootcause.*fix/.test(r))).toBe(
      true,
    );
  });
  ...
  it("有 R 但缺 V 复审 rootcause 时 R7 时序校验也失败", async () => {
    ...
    expect(
      result.violations.some((r) =>
        /R7.*rootcause.*review.*targetKind=rootcause/.test(r),
      ),
    ).toBe(true);
  });
});
...
describe("run-log E7: gateExitCode 未回填", () => {
  it("gateLogPath 已设但 gateExitCode 为 null 应被 R6 拦截", async () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.violations.some((r) => /R6.*gateLogPath.*gateExitCode/.test(r)),
    ).toBe(true);
  });
});
...
describe("run-log E9: 1 fix 可覆盖多份 R 报告（去重映射）", () => {
  ...
  it("2 份 R 报告但仅 1 份有 fix 应被 R3 拦截", async () => {
    ...
    expect(
      result.violations.some((r) =>
        /R3.*rootcause.*RC-phase5-1-02.*无对应 fix/.test(r),
      ),
    ).toBe(true);
  });
});
...
describe("run-log R8 扩展：S-fix/emergency-fix 后须 R3", () => {
  it("S-fix 后无 R3 直接 V 应失败", () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.violations.some((v) => /R3 记录校验失败.*S\(fix\)/.test(v)),
    ).toBe(true);
  });
  ...
  it("S-emergency-fix 后无 R3 直接 V 应失败", () => {
    ...
    expect(
      result.violations.some((v) =>
        /R3 记录校验失败.*S\(emergency-fix\)/.test(v),
      ),
    ).toBe(true);
  });
  ...
  it("S-fix 后有 3 条 R3 再 V 应通过 R8（不因 fix 段报违规）", () => {
    ...
    const result = checkRunLog(entries, { gateLogs: new Map() });
    expect(result.violations.some((v) => /R3 记录校验失败/.test(v))).toBe(
      false,
    );
  });
...
describe("B1 gate-log 根 JSON 协议", () => {
  ...
  it.each([0, 1, 2])(
    "extractExitCode 从根 JSON 提取 exitCode=%s",
    (exitCode) => {
  ...
  it("根 JSON 与 passed 不一致返回可追踪 violation，且不提取为合法 exitCode", () => {
    ...
    expect(
      inspected.violations.some((v) =>
        /passed.*exitCode|exitCode.*passed/.test(v),
      ),
    ).toBe(true);
...
function makeEntry(overrides: Partial<RunLogEntry>): RunLogEntry {
  ...
  if (merged.action === "rootcause") {
    ...
    if (merged.rootCauseCategory === undefined)
      merged.rootCauseCategory = "coding-error";
    if (merged.upstreamDefect === undefined) merged.upstreamDefect = false;
    if (merged.rollbackRecommended === undefined)
      merged.rollbackRecommended = false;
  }
...
describe("run-log R8 轨迹模板校验（agentic Ch19 轨迹符合性）", () => {
  it("已完成阶段 gate 在 checkpoint 之后 → 违规", () => {
    ...
    const result = checkRunLog(lines);
    expect(result.violations.some((v) => /R8.*gate.*checkpoint/.test(v))).toBe(
      true,
    );
  });
  ...
  it("R8-4: V(review) 先于任何 S 变体 → 轨迹顺序倒置违规", () => {
    ...
    const result = checkRunLog(lines);
    expect(
      result.violations.some((v) => /R8.*轨迹顺序倒置.*V\(review\).*S/.test(v)),
    ).toBe(true);
  });
  ...
  it("R8-4: R3 先于任何 S 变体 → 轨迹顺序倒置违规", () => {
    ...
    const result = checkRunLog(lines);
    expect(
      result.violations.some((v) => /R8.*轨迹顺序倒置.*S.*R3/.test(v)),
    ).toBe(true);
  });
  ...
  it("R8-4: gate 先于 V(review) → 轨迹顺序倒置违规", () => {
    ...
    const result = checkRunLog(lines);
    expect(
      result.violations.some((v) => /R8.*轨迹顺序倒置.*V\(review\).*G/.test(v)),
    ).toBe(true);
  });
...
describe("D8 lifecycle identity reducer", () => {
  ...
  const r3 = (
    ...
    makeEntry({
      ...
      implementationTarget: reportId
        ? (implementationTarget ?? `implementation-${reportId}`)
        : undefined,
      target: reportId
        ? (implementationTarget ?? `implementation-${reportId}`)
        : undefined,
  ...
  it("does not let a different reportId rootcause V/G or fix satisfy the target lifecycle", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /RC-B.*open-approved-lifecycle/.test(reason),
      ),
    ).toBe(true);
    expect(
      result.violations.some((reason) =>
        /RC-A.*pending-pre-approval/.test(reason),
      ),
    ).toBe(false);
  });
  ...
  it("does not count rootcause V as implementation V", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /implementation.*V|缺.*implementation|缺.*实现.*V/i.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("classifies an unapproved report without exact fix as pending-pre-approval", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /RC-A.*无对应 fix|exact-fix|open-approved-lifecycle/.test(reason),
      ),
    ).toBe(false);
    expect(
      (result as unknown as { diagnostics?: string[] }).diagnostics?.some((d) =>
        /pending-pre-approval/.test(d),
      ),
    ).toBe(true);
  });
  ...
  it("reports open-approved-lifecycle when same-identity rootcause V/G passed but exact fix is absent", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /open-approved-lifecycle/.test(reason),
      ),
    ).toBe(true);
    expect(
      result.violations.some((reason) => /RC-A.*无对应 fix/.test(reason)),
    ).toBe(false);
  });
  ...
  it("counts R3 only inside the same fix identity segment", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /RC-B|fix.*RC-B|implementation.*RC-B/i.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("checks R8 ordering within an identity segment instead of phase-wide first indexes", () => {
    ...
    const result = checkRunLog(entries);
    expect(
      result.violations.some((reason) => /R8.*轨迹顺序倒置/.test(reason)),
    ).toBe(false);
    expect(
      result.violations.some((reason) => /R3 记录校验失败/.test(reason)),
    ).toBe(false);
  });
  ...
  it("keeps missing implementation V blocking after an exact fix", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /fix f-a.*缺同身份 implementation V/.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("keeps missing implementation G blocking after same-identity R3 and V", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /fix f-a.*缺同身份 implementation G/.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("emits LEGACY_UNSCOPED diagnostics when lifecycle identity fields are missing", () => {
    ...
    expect(
      (result as unknown as { diagnostics?: string[] }).diagnostics?.some((d) =>
        /LEGACY_UNSCOPED|deferred/i.test(d),
      ),
    ).toBe(true);
  });
  ...
  it("defers the raw legacy fix without exact identity blocking or lifecycle credit", async () => {
    ...
    expect(
      result.violations.some((reason) =>
        /exact fix identity incomplete/.test(reason),
      ),
    ).toBe(false);
    expect(
      result.violations.some((reason) =>
        /RC-phase8-10-01.*implementation V|同身份 R3/.test(reason),
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      diagnostics.some((diagnostic) =>
        /pending-pre-approval: RC-phase8-2-02/.test(diagnostic),
      ),
    ).toBe(true);
  });
  ...
  it("rejects implementation evidence that targets a different implementationTarget", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /same identity|implementationTarget|缺同身份 implementation V/i.test(
          reason,
        ),
      ),
    ).toBe(true);
  ...
  it("rejects a rootcause-to-fix relation whose fix reportId differs from the root report", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /R7.*RC-A.*exact.*fix|reportId/i.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("does not approve a rootcause review with missing identity fields", () => {
    ...
    expect(
      result.diagnostics?.some((diagnostic) =>
        /pending-pre-approval/.test(diagnostic),
      ),
    ).toBe(true);
  });
  ...
  it("does not count an R3 record with missing implementation identity", () => {
    ...
    expect(
      result.diagnostics?.some((diagnostic) =>
        /LEGACY_UNSCOPED.*implementationTarget/.test(diagnostic),
      ),
    ).toBe(true);
  });
  ...
  it("rejects duplicate R3 dimensions instead of counting a Set of three names", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /R3 记录校验失败|duplicate|恰好一条/i.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("does not let a missing-identity legacy fix satisfy a strict phase 8 lifecycle", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /open-approved-lifecycle|exact.*fix|identity/i.test(reason),
      ),
    ).toBe(false);
    expect(
      result.diagnostics?.some((diagnostic) =>
        /LEGACY_UNSCOPED/.test(diagnostic),
      ),
    ).toBe(true);
    expect(
      result.diagnostics?.some((diagnostic) => /deferred/.test(diagnostic)),
    ).toBe(true);
  });
  ...
  it("rejects an implementationTarget with the wrong schema type", () => {
    ...
    expect(result.passed).toBe(false);
    expect(
      result.violations.some((reason) => /implementationTarget/.test(reason)),
    ).toBe(true);
  });
  ...
  it("keeps a phase-5 legacy segment on legacy rules when phase 8 has a strict segment", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /R3 记录校验失败.*阶段 5/.test(reason),
      ),
    ).toBe(true);
    expect(
      result.violations.some((reason) => /identity segment/.test(reason)),
    ).toBe(false);
  });
  ...
  it("does not treat a failed phase-5 fix as a legacy R7 chain head", () => {
    ...
    expect(
      result.violations.some((reason) => /R7.*rootcause.*fix/.test(reason)),
    ).toBe(true);
    expect(
      result.diagnostics?.some((diagnostic) =>
        /NON_CREDIT_FIX.*legacy-failed-fix/.test(diagnostic),
      ),
    ).toBe(true);
    expect(result.lifecycleStatus).toBe("NOT_CLOSED_NOT_PROVEN");
  });

  it.each(["fail", "blocked", "cancelled"] as const)(
    "does not open R3/R8 credit for a %s fix",
    (outcome) => {
      const entries = [
      ...
      expect(
        result.diagnostics?.some((diagnostic) =>
          diagnostic.includes(
            `NON_CREDIT_FIX: fix f-failed outcome=${outcome}; credit deferred`,
          ),
        ),
      ).toBe(true);
    },
  ...
  it("requires R7 fix target and artifacts to match the exact implementation target", () => {
    ...
    const result = checkRunLog([
      rootCause("r-target", "RC-TARGET"),
      rootCauseReview("v-target", "RC-TARGET"),
      badFix,
    ]);
    expect(
      result.violations.some((reason) =>
        /R7.*exact.*target|R7.*artifacts/.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("keeps an incomplete legacy fix diagnostic-only instead of granting lifecycle credit", () => {
    ...
    expect(
      result.diagnostics?.some((diagnostic) =>
        /LEGACY_UNSCOPED.*legacy-fix-only/.test(diagnostic),
      ),
    ).toBe(true);
    expect(
      result.diagnostics?.some((diagnostic) =>
        /LEGACY_UNSCOPED.*legacy-fix-only.*credit deferred/.test(diagnostic),
      ),
    ).toBe(true);
  });
  ...
  it("does not let an unrelated strict fix truncate the current identity window", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /R3 记录校验失败.*fix fix-a/.test(reason),
      ),
    ).toBe(false);
  });
  ...
  it("requires a failed implementation review to be followed by a same-segment rootcause path", () => {
    ...
    expect(
      result.violations.some((reason) =>
        /R8.*V\(review\) 失败.*直接 S/.test(reason),
      ),
    ).toBe(true);
  });
  ...
  it("does not let phase-8 identity-incomplete evidence cover a legacy rootcause report", () => {
    ...
    expect(
      result.violations.some((reason) => /RC-LEGACY.*无对应 fix/.test(reason)),
    ).toBe(false);
    expect(
      result.diagnostics?.some((diagnostic) =>
        /LEGACY_UNSCOPED.*deferred/.test(diagnostic),
      ),
    ).toBe(true);
  });
});
...
describe("RunLogEntry schema/type contract", () => {
  it("keeps the TypeScript action union and lifecycle fields aligned with the live schema", async () => {
    const schema = JSON.parse(
      await fs.readFile(
        path.join(here, "..", "..", "schemas", "run-log.schema.json"),
        "utf8",
      ),
    ) as { properties: { action: { enum: string[] } } };
    const source = await fs.readFile(
      path.join(here, "..", "logic", "run-log-logic.ts"),
      "utf8",
    );
    const actionBody =
      source.match(/action:\r?\n([\s\S]*?)\r?\n\s+role:/)?.[1] ?? "";
    const typeActions = [...actionBody.matchAll(/["']([^"']+)["']/g)].map(
      (match) => match[1],
    );
...
describe("run-log action-role 配对（blocking，logic 层强制）", () => {
  ...
  it("emergency-fix 由非 S 角色执行 → blocking violation", () => {
    const result = checkRunLog([
      baseEntry("emergency-fix", "O", "run-emergency"),
    ]);
    expect(result.passed).toBe(false);
  ...
  it("合法 action-role 配对不产生 action-role violation", () => {
    ...
    const result = checkRunLog(entries);
    expect(
      result.violations.some((v) => v.startsWith("action-role 配对")),
    ).toBe(false);
  });
});
...
describe("run-log emergency-fix variant 语义", () => {
  it("带 variant=emergency-fix + blocker 的 emergency-fix 记录 schema-valid 且无 [schema] violation", () => {
    ...
    expect(
      result.violations.some(
        (v) => v.startsWith("条目") && v.includes("[schema]"),
      ),
    ).toBe(false);
  });
  ...
  it("无 variant 的旧 emergency-fix 记录 → LEGACY diagnostic 而非 blocking [schema]", () => {
    ...
    expect(result.diagnostics?.some((d) => /LEGACY_VARIANT/.test(d))).toBe(
      true,
    );
    expect(result.lifecycleStatus).toBe("NOT_CLOSED_NOT_PROVEN");
  });
  ...
  it("带 variant=emergency-fix 但缺 blocker → blocking（声明了紧急通道却无阻塞原因）", () => {
    ...
    const result = checkRunLog([entry]);
    expect(result.passed).toBe(false);
    expect(result.violations.some((v) => /\[schema\].*blocker/.test(v))).toBe(
      true,
    );
  });
  ...
  it("双 legacy（phase-8 缺 identity + 缺 variant/blocker）→ LEGACY 吸收：非 blocking + NOT_CLOSED + LEGACY_VARIANT/LEGACY_UNSCOPED", () => {
    ...
    const variantDiag = result.diagnostics?.find((d) =>
      /LEGACY_VARIANT/.test(d),
    );
    ...
    expect(
      result.diagnostics?.some(
        (d) => /LEGACY_UNSCOPED/.test(d) && d.includes("identity missing"),
      ),
    ).toBe(true);
  });
...
describe("run-log reworkHints 强制（LEGACY_REWORK_HINTS cutoff 分界）", () => {
  ...
  it("cutoff 后 review passed=false 无 reworkHints → blocking", () => {
    const result = checkRunLog([
      failedReview({ timestamp: "2026-09-02T00:00:00.000Z", passed: false }),
    ]);
    expect(result.passed).toBe(false);
    expect(
      result.violations.some((v) =>
        /\[rework-hints\].*passed=false.*reworkHints/.test(v),
      ),
    ).toBe(true);
    expect(
  ...
  it("cutoff 前 review passed=false 无 reworkHints → LEGACY_REWORK_HINTS 诊断放行", () => {
    const result = checkRunLog([
      failedReview({ timestamp: "2026-08-01T00:00:00.000Z", passed: false }),
    ]);
    expect(result.passed).toBe(true);
    expect(
      result.violations.some(
        (v) => v.includes("[schema]") || v.includes("[rework-hints]"),
      ),
    ).toBe(false);
    expect(result.diagnostics?.some((d) => /LEGACY_REWORK_HINTS/.test(d))).toBe(
      true,
    );
    expect(result.lifecycleStatus).toBe("NOT_CLOSED_NOT_PROVEN");
  });
  ...
  it("passed=false 且 reworkHints 非空 → 正常（无该规则违规）", () => {
    ...
    expect(
      result.violations.some((v) =>
        /\[rework-hints\]|\[schema\].*reworkHints/.test(v),
      ),
    ).toBe(false);
  });
});
...
describe("A-3d 跨轮次评审一致性（R9 标准偏移）", () => {
  ...
  it("同一产物两次 review 只差 1 档 → 不触发（产物确实可能改进了）", () => {
    ...
    expect(out.violations.some((v) => v.includes("跨轮次评审不一致"))).toBe(
      false,
    );
  });
  ...
  it("四条记录 A→C 跨多轮 → 只报一次（按产物聚合，不重复报）", () => {
    ...
    expect(
      out.violations.filter((v) => v.includes("跨轮次评审不一致")),
    ).toHaveLength(1);
  });
});
...
describe("run-log R10: revertEvidence 回滚证伪（严格证据模式）", () => {
  ...
  it("fix 缺 revertEvidence → R10 blocking（无 LEGACY 吸收）", async () => {
    const result = checkRunLog(
      stripRevertEvidence(await loadShiftedPastCutoff()),
    );
    expect(result.passed).toBe(false);
    expect(
      result.violations.some(
        (v) => v.startsWith("R10:") && v.includes("revertEvidence"),
      ),
    ).toBe(true);
    expect(
      result.diagnostics?.some((d) => d.startsWith("LEGACY_REVERT_EVIDENCE")) ??
        false,
    ).toBe(false);
  ...
  it("fix command 仅空白 → schema 阻断，不能以字符串存在替代有效命令", async () => {
    ...
    expect(
      result.violations.some(
        (v) => v.includes("[schema]") && v.includes("command"),
      ),
    ).toBe(true);
  });
  ...
  it("R10: cutoff 前 fix 缺 revertEvidence 仍阻断，timestamp 不得作为 legacy 放行", async () => {
    const result = checkRunLog(
      stripRevertEvidence(await loadJsonl("rootcause-valid.jsonl")),
    );
    expect(result.passed).toBe(false);
  ...
  it("旧日期 emergency-fix 缺 revertEvidence → R10 blocking 且 legacy=0", () => {
    ...
    expect(
      result.violations.some(
        (v) => v.startsWith("R10:") && v.includes("emergency-fix"),
      ),
    ).toBe(true);
    expect(result.revertEvidence).toEqual({
      checked: 1,
