...
| docs-consistency-logic.test.ts | DocConsistency | 活体文档计数（schema / CLI / exit2 / references / persona / test files / vitest）/ 版本七处一致 / targetKind 废弃标记 / DoD 七维度 / 反模式区间 #1~#48 / baseline-sync / evidence-provenance 与 wm-verify-evidence-source 登记 |
...
| verifier-logic.test.ts | Verifier | 4 个 Persona fixture 逐一通过当前 Schema/logic / evidence 格式校验（key=value 通过·空泛声明 O3）/ R13 单轴下限（≥0.70·<0.70·边界 0.70·非数组）/ evidence 扣分后 passed 重算 / targetKind=rootcause（§7.5 合法通过·误用 test 集合拦截·权重改动拦截·非法值拦截） |
