 ...
 *   - 复用 infrastructure/schema-loader.ts 的 Ajv 单例与 schemas/ 自动加载机制
 ...
 *     （ERROR_JSON 已输出）；其他真实异常（如 infrastructure/schema-loader 的 getAjv 在 schemas 目录缺失 /
 ...
 * 前缀的哨兵 Error；调用方必须 catch 哨兵错误（return 2），真实异常（infrastructure/schema-loader /
 * 环境级错误）应继续抛出，不得吞掉。
 *
