 ...
 *
 * uat-path-mapping 相关逻辑：
 * - parseUatPathMappingFromContent：严格解析
 * - checkUatPathMappingContent：综合校验（严格解析 + 回填校验）
...
/**
 * 从 uat-path-mapping.md 内容解析映射行（严格解析）。
 ...
 *
 * 实现：统一复用 design-contract-logic.parseUatPathMappingContent（strict=true），
 * 字段映射到 uatId/actualPath/mappingType；violation 文案与行号格式与严格解析逐字节一致。
 */
...
/**
 * uat-path-mapping 内容综合校验（严格解析 + 回填校验）。
 * 供 check-artifact-gate CLI 阶段 5 / 终检与 self-test 共用。
 */
...
/**
 * 汇总 uat-path-mapping 校验违反（计入终检结果：解析严格化 + 阶段5/终检均校验）：
 * - phase=1：校验 docs/uat-path-mapping.md 存在性（P0-1）；
 * - phase=5 / 终检（无 --phase 终检默认 phase 8 也校验）：读取并回填校验。
 */
export async function collectUatMappingViolations(
  ...
  // P0-1: phase=5 / 终检（无 --phase 终检默认 phase 8 也校验）校验 uat-path-mapping 回填
