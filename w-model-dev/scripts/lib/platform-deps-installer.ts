...
function fail(code: PlatformDependencyVerificationError['code'], message: string): never {
...
export function resolveLockfileV3Package(lockfile: string | Buffer, packageName: string): LockedPackage {
  const parsed = parseLockfile(lockfile);
  if (parsed.lockfileVersion !== 3) {
    fail('lockfile-version', '仅接受 npm lockfileVersion 3');
  }

  const packageEntry = asRecord(parsed.packages?.[`node_modules/${packageName}`]) as LockfilePackage | undefined;
  if (
    !packageEntry ||
    ...
    fail('lockfile-package', `lockfile 未提供 ${packageName} 的 version/resolved/integrity`);
  }
...
function validateResolvedUrl(resolved: string, allowedRegistryHosts: readonly string[]): void {
  let url: URL;
  try {
...
function readPackageManifest(entries: readonly ArchiveEntry[]): PackageManifest {
  const packageJson = entries.find((entry) => entry.path === 'package/package.json' && entry.type === 'file');
  ...
  try {
    const parsed: unknown = JSON.parse(
      Buffer.isBuffer(packageJson.content) ? packageJson.content.toString('utf8') : packageJson.content,
    );
...
function validatePackageManifest(manifest: PackageManifest, locked: LockedPackage): void {
  ...
  if (manifest.version !== locked.version) {
    fail('package-version', `package/package.json version 与 ${locked.version} 不一致`);
  }
}
...
export async function verifyPlatformDependency(
  ...
  const locked = resolveLockfileV3Package(options.lockfile, options.packageName);
  ...
  try {
    ...
    const loadedModule = await options.loadModule(path.join(temporaryDirectory, 'package'));
