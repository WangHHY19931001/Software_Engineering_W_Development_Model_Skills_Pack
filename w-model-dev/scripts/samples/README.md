...
| `graph` | check-requirement-graph | GRAPH_CASES（31）+ ENHANCE×4（16） | 图谱 R1-R15 + 规格/大纲/详细设计增强 | 平铺 JSON |
...
合计 285 条用例 + 1 条 metadata 用例 = **286 条**（`self-test.ts` 基线）。用例数与「对应 check 脚本」列的数组条数不一致时以 self-test.ts 为准（数组条目数 = 实际执行数）。
