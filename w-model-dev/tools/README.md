...
**权威版本记录**：`references/tla-plus.md`「工具链」节（实测确认方法：`java -cp tla2tools.jar tlc2.TLC` 输出 `TLC2 Version 2.19 of 08 August 2024`）。

**同步策略**：手动同步，随 skill 版本演进更新（升级时同步更新本表与本仓库 README 中相关版本声明）；不采用每次运行自动下载——保证 `check-tla-model.ts` 离线可用、结果可复现。

**依赖**：宿主 Java runtime ≥ 11（唯一外部依赖，见 tla-plus.md「工具链」节）。
