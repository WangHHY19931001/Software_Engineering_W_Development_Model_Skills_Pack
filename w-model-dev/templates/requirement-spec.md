...
> **行为规格承接**：行为规格由独立 `.feature` 文件承载（bdd.md §2 头规范管），本规格 §16 引用块指向的 behavior-spec.md 定义引用关系，不内联 feature 块。
...
| REQ-002 | 2 | P0 | REQ-001 | REQ-001 | module | {{模块描述}} | — | {{可选: 依据文档/用户原话}} |
| REQ-003 | 3 | P1 | REQ-001 | REQ-002 | feature | {{功能点描述}} | — | {{可选: 依据文档/用户原话}} |
| REQ-004 | 4 | P1 | REQ-001 | REQ-003 | acceptance | {{验收标准描述}} | {{可量化标准}} | {{可选: 依据文档/用户原话}} |
| NFR-001 | {{1-4}} | P{{0-3}} | {{level=1 祖先}} | {{parent}} | NFR | {{描述}} | {{指标}} | {{可选: 依据文档/用户原话}} |
| CON-001 | {{1-4}} | P{{0-3}} | {{level=1 祖先}} | {{parent}} | CON | {{描述}} | {{约束}} | {{可选: 依据文档/用户原话}} |
...
> 显式声明排除的功能/场景；覆盖缺失项（§7 未覆盖）须在此声明并关联豁免审批（如 `EXEMPT-NNN`）。
> 本节同时是阶段 1 迷雾项「判 Out of Scope」毕业后的**持久拒绝登记**（概念粒度 + `Prior requests` 回链）；登记规则的权威细则见 `references/phase-1-requirements.md`「迷雾登记册（Fog of War）」节。

**强制项**：至少 1 行——即使是「无」也要显式声明（删除示例行后只保留 `-` 行，不得留白或删除本表；示例行 `dark-mode` 仅为形态示范，须替换为本项目实际登记行）；Brownfield 项目须明确不动哪些历史模块（逐模块登记为一行）。

| conceptKey | 拒绝理由 | Prior requests | 状态 | 来源 |
| --- | --- | --- | --- | --- |
| `dark-mode` | 渲染管线假设单色板，主题切换与既有品牌规范冲突 | `REQ-102`, `REQ-118` | `rejected` | `阶段1` |
| `-` | 本阶段无排除项（显式「无」） | `-` | `-` | `-` |

**登记规则速览**（权威细则见 `references/phase-1-requirements.md`「迷雾登记册（Fog of War）」节）：
- **概念粒度**：一概念一行；同一概念的多个请求归并入该行的 `Prior requests`，不另开行。
- **`conceptKey` 唯一**：kebab-case；节内不得重复。
- **`Prior requests` 回链**：逗号分隔的非空标识列表（`REQ-xxx` / `<轮次>-<序号>` / 请求原文短引）；无回链时写 `-`（显式空，不得留白）。
- **不删除行**：改主意时把 `状态` 改为 `reconsidered`，并在 `拒绝理由` 列写明去向（`→ REQ-xxx` 或 `→ 见 §8.5`）；不得删除或清空既有行。
- **仅 rejected enhancement 入册**：bug / 缺陷不入册；已实现者拒收（登记已实现项会污染去重，改用指向既有实现的引用）。
