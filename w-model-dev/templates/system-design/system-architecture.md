# 系统架构（System Architecture）

> 对应 DESIGN.md §5 顶层架构 + §8 系统行为总览 + §9 运行时架构。系统级设计：
> 组件图 / 子系统清单 / 系统树 / 架构原则 / ADR / 行为总览 / 运行时架构。
> **阶段边界**：本文件只产系统级设计，不落接口契约（阶段 3）与类/方法定义（阶段 4），越界即返工（FM-SD-06）。
> 模板版本：v1.0。主文档引用块：`> 系统架构详见 [{{module}}-system-architecture.md](./{{module}}-system-architecture.md)`。

## 1. 顶层组件图

> 体现分层 + 组件间依赖 + 数据流（须含 `-.->` 或 `>>` 数据流箭头，缺则 FM-SD-01）。

```mermaid
graph TB
  {{组件1}} -->|依赖| {{组件2}}
  {{组件2}} -.->|数据流| {{组件3}}
```

## 2. 规范性子系统清单

| 子系统 ID | 名称 | 职责 | 涉及模块 ID |
|---|---|---|---|
| S-{{xx}} | {{名称}} | {{职责}} | M-{{xx}} |

> 强制：子系统 ID 与主模板 §3 模块划分一一对应（R9 门禁校验；不对应 → 回步骤 3）。

## 3. 系统树

```mermaid
graph TD
  {{系统}} --> {{子系统1}}
  {{系统}} --> {{子系统2}}
```

## 4. 架构原则

- {{原则 1}}（例：分层单向依赖，禁止反向依赖）
- {{原则 2}}

## 5. 架构决策记录（ADR）

| ADR 编号 | 决策 | 上下文 | 后果 |
|---|---|---|---|
| ADR-{{xx}} | {{决策}} | {{上下文}} | {{后果（含代价/收益）}} |

> 强制：每条 ADR 有决策 + 上下文 + 后果（缺则 FM-SD-02）。
> **门禁强制**：`check-artifact-gate.ts --phase=2 --spec-dir=<dir>` 的 `adr` 桶逐行校验本表——`决策` / `上下文` / `后果` 三列均不得为空（`{{...}}` 占位符、`—`、`-` 视为未填）。**判据边界**：门禁只判「表存在时三列非空」，**不判是否该有 ADR**（ADR 入选三问见 `phase-2-system-design.md`），故表为空或缺节不报。

## 6. 系统行为总览

- 对外行为：{{系统向外部提供的可观察行为}}
- 对内行为：{{子系统间内部协作行为}}
- 关键分叉行为：{{需要人参与/升级/终止的关键分叉}}

## 7. 运行时架构

- 运行时组件：{{运行时主要组件清单}}
- 核心对象（概念级）：{{运行时核心对象，仅概念级描述}}
