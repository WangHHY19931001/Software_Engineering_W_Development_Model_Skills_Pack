# 系统测试文档

> 阶段 7（系统测试）产出。套用时替换 `{{}}` 占位符。
>
> 系统测试聚焦端到端业务链路、安全约束、性能基线与异常路径验证。

## 文档信息

- 项目名称：{{项目名称}}
- 文档版本：{{v1.0}}
- 执行日期：{{YYYY-MM-DD}}
- 执行者：{{执行者}}

## 1. 测试范围

{{系统测试范围：端到端业务链路、安全约束、性能基线、异常路径等。}}

## 2. 测试用例

| 用例 ID | 标题 | 关联需求 | 优先级 | 状态 | 备注 |
|---|---|---|---|---|---|
| {{TC-SYS-001}} | {{}} | {{REQ-NNN}} | 高 | ✅ 通过 | {{}} |

## 3. 性能结果

| 指标 | 目标 | 实测 | 是否达标 |
|---|---|---|---|
| 响应时间 | {{目标}} | {{}} | ✅/❌ |
| 并发承载 | {{}} | {{}} | ✅/❌ |
| 错误率 | 0% | {{}} | ✅/❌ |

## 性能度量环境声明

> 性能测试须声明测试环境与对应阈值。

### 测试环境类型

| 环境类型 | 说明 | 典型放宽倍数 |
|---|---|---|
| CI | 持续集成环境（GitHub Actions / GitLab CI 等），资源受限，并发跑多任务 | 生产目标的 2-3 倍 |
| full-suite | 完整测试套件环境（本地或专用测试机），资源较充裕 | 生产目标的 1.5-2 倍 |
| isolated | 隔离性能测试环境（专用性能测试机，无其他任务干扰） | 生产目标的 1-1.2 倍 |

### 性能度量环境声明模板

| NFR-ID | CI 阈值 | full-suite 阈值 | isolated 阈值 | 生产目标值 |
|---|---|---|---|---|
| NFR-001 | p95 ≤ 400ms | p95 ≤ 300ms | p95 ≤ 250ms | p95 ≤ 200ms |
| NFR-002 | p99 ≤ 1000ms | p99 ≤ 800ms | p99 ≤ 600ms | p99 ≤ 500ms |

**度量工具**：[如 k6 / wrk / autocannon]
**度量方法**：[如 1000 并发持续 60s，取 p95/p99]
**环境配置**：[如 CI: 2vCPU/4GB RAM; isolated: 8vCPU/16GB RAM]

### 校验

- `check-artifact-gate.ts --phase=7 --scope=<change-scope.json>`：系统测试列回填、已通过测试层级与 NFR 字段校验，退出码 0。
- `check-bdd-model.ts --phase=7 --graph=.w-model/ingestion/graph.json --require-cucumber-report --cucumber-report=reports/cucumber/system.json`（L2 features 时）：退出码 0；真实 Cucumber 报告缺失即阻断。
- `check-artifact-gate.ts` 在 NFR 类型 RTM 行校验 `targetValue` + `testThreshold` 字段存在性（双字段均缺失 → blocking violation / exit 1；单缺其一不阻断）。
