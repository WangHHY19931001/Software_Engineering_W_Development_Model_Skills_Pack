...
阶段 4 TLA+ 零违反 + 图谱零违反才放行进编码。TLA+ 不接受占位/简化/错误实现（反模式 #16）；建模须符合需求和设计，符合后仍有问题须修正需求/设计并回退重跑（反模式 #17）；BDD↔TLA+ 不等价必须走普通 V/G 失败链（见「普通 V/G 失败链」节）（反模式 #29）。详见 [tla-plus.md](tla-plus.md) 与 [bdd.md](bdd.md)。
...
| 检测信号 | Agent 自查信号 + 命中后回退命令 | 「检测信号与回退命令」节 |
| 命中处理 | 停止 → 回退 → 告知用户 → 记录教训 | 「命中后的处理流程」节 |

**按场景只读 §X**：

| 场景 | 应读章节 |
|---|---|
| 进入阶段 N 前对照高发陷阱 | 「阶段 N 必读反模式清单」节 |
| 定位某反模式与硬约束的关系 | 「反模式-硬约束映射」节 |
| 反模式由哪个脚本守护 / 退出码含义 | 「与门禁脚本的对应关系」节 + 「门禁脚本退出码精确对应表」 |
| 自查是否命中 + 命中后回退命令 | 「检测信号与回退命令」节 |
| 命中后处理步骤 | 「命中后的处理流程」节 |
| 单条反模式深入（#14~#17 / #29 / #30 等） | 对应「## #N」强化节 |

### 阶段 N 必读反模式清单

> 由下方「命中高发阶段」表反向重排为「阶段 → 反模式编号」视角，供 Agent 进入阶段 N 前快速对照该阶段高发陷阱。命中任一条即视为流程破坏，必须回退到对应阶段起点。

| 阶段 | 高发反模式编号 | 一句话检测信号 |
|---|---|---|
| 阶段 1（需求分析） | #2、#11、#12、#13、#14、#15、#16、#17、#26、#27、#28、#29、#30、#37 | 测试设计后置；ingestion 未跑图谱校验 / A 自评收敛 / 信息流黑洞放行；TLA+ 语法/行为/占位/不符未回退；RunLog 与 EventIngress 字段混用；调测者简化；schema 前置校验缺失；BDD 建模不符未回退；豁免审批跳步；产物膨胀核心决策稀疏 |
...
写指引前先分类**基线失败**：对一种失败类型「防弹」的表述形式，在另一种失败类型上会**可测地反噬**。反模式清单的三列（反模式（不要做）/ 危害 / 正确做法）只产出一个形式——禁令；当基线失败不是「明知故犯」时，禁令会加重而非消除该失败。按失败类型选形式：

| 基线失败 | 正确形式 | 错误形式 |
|---|---|---|
| 压力下明知故犯（知道规则，却在压力下违反） | 禁令 + rationalization 表 + 红旗信号 | 软指引（"prefer…" / "consider…"） |
| 遵循了但输出形状错（臃肿的 prompt、被埋的判定、重述的规格） | 正向 recipe 或契约：说明输出**是什么**——它的部件，按序 | 禁令清单（"don't restate" / "never narrate"） |
| 漏掉已产出物里的必填元素 | 结构性：把他们本来就要填的模板里放上 REQUIRED 字段或槽位 | 模板旁边的散文提醒 |
| 行为应当取决于某条件 | 以可观测谓词为键的条件句（"如果 brief 存在，就引用它"） | 无条件规则 + 豁免子句 |

**两条硬子规则（无论选哪种形式都适用）：**

- **No nuance clauses**：`Don't X unless it matters` 会重新打开谈判——真实例外必须写成以可观测谓词为键的独立条件句，不给既有规则追加尾巴。
- **Exemption clauses don't scope**：`该限制不适用于代码块` **仍然**会压制代码块；输出的某部分必须豁免时，重构到规则够不着它。

**接地说明：**

- 本仓库 48 条反模式的**主表述本轮不改写**。依据三重且相互加强：① 规则自洽——任一合理分类口径下，全量改写都会对 discipline 型条目用错形式，违反分流判据自身；② 对照臂未复现失败——没有可复现的缺陷可供消除；③ 量化证据缺失。依据文件：`docs/superpowers/specs/2026-09-14-d1-local-ab-evidence.md` §5.3。
- 来源文声称「禁令臂产出劣于无指引对照（分布完全分离）」的**量化排序属未验证假设**：该主张无随仓可复核证据，本地 A/B 因 treatment×metric 混淆未能裁定；本仓库**不采纳**该量化排序。
- 本仓库采纳的是**形状判据**：上表四行与两条硬子规则是**可自证**的判别规则，正常落地，不标为假设。
- 逐字依据：`docs/superpowers/sources/2026-09-14-superpowers-adopted-excerpts.md` §S01（四行对照表 + 两条硬子规则）；`docs/superpowers/sources/2026-09-14-mattpocock-skills-adopted-excerpts.md` §M02（整体否定式表述会把被禁行为拖入上下文并提高其可用性：应写正面目标，硬护栏保留禁令时须配正面目标）。
...
| #14（TLA+ 跳过语法检查） | 阶段 1~4 | [tla-plus.md](tla-plus.md)「编码调试顺序」节 |
| #15（TLA+ 死锁/违反放行） | 阶段 1~4 | [tla-plus.md](tla-plus.md)「校验脚本」节 |
| #16（TLA+ 占位/简化/错误实现） | 阶段 1~4 | [tla-plus.md](tla-plus.md)「合规性约束」节 |
| #17（TLA+ 与需求/设计不符未回退） | 阶段 1~4 | [tla-plus.md](tla-plus.md)「建模与需求/设计一致性」节 |
...
| #29（BDD 建模不符未回退） | 阶段 1-4 | [bdd.md](bdd.md)「不符处理流程」节 |
...
| #28（schema 前置校验缺失） | [`schema-loader.ts`](../scripts/infrastructure/schema-loader.ts) `validateBySchema` 调用检测（`*-logic.ts` 入口未 import / 未调用即命中）+ 错误信息缺 `[schema]` 前缀检测 + [data-models.md](data-models.md)「JSON Schema 强约束」节 schema 清单（34 份，含 change-scope / codegraph-query 与 evidence-manifest / evidence-provenance） |
...
| #3 | 质量门节点未执行 `check-artifact-gate.ts` / 仅 LLM 文本说「通过」 | 立即执行 `npx tsx w-model-dev/scripts/cli/check-artifact-gate.ts [project-dir] --phase=<N>`；阶段 5-8 另须 `--scope=<change-scope.json>`，scope 缺失或与 Git 实际变更不符即 exit 1（fail-closed 设计）；退出码 1 先走普通 V/G 失败链（hard-constraints），再按 R 结论由 S-fix 回阶段 5；退出码 2 仅修正输入后重跑 | `check-artifact-gate.ts` 退出码 0=通过 / 1=未通过 / 2=输入错误 |
| #4 | `VerifierOutput.passed=false` 但 `Project.status` 已推进到下一阶段 | 将 `reworkHints` 仅作为 R 定位线索；完成普通 V/G 失败链后，按 R 的结论回到对应阶段并重置 `status` 字段 | `check-verifier-output.ts` 退出码 0 + `passed=true`；完整链为普通 V/G 失败链（见「普通 V/G 失败链」节） |
...
| #7 | `check-artifact-gate.ts` 退出码 1/2 但 `Project.status` 已标「验收通过」 | 先将退出码 1 记录为 R 定位线索，完成普通 V/G 失败链后按 R 结论由 S-fix 修复；退出码 2 仅先修正输入再重新执行，不得将输入错误当作放行依据；随后重置 `status` 为「编码」并由用户在 CHECKPOINT 确认 | 退出码 1/2 → 一律不得放行；完整链为普通 V/G 失败链（见「普通 V/G 失败链」节） |
...
| #15 | `TLA_JSON.passed=false`（deadlockViolations/invariantViolations/stateExplosionSpecs 非空）但阶段已推进 | 先将问题记录为 R 定位线索；完成普通 V/G 失败链后，按 R 结论由 S-fix 修正 TLA+ 规格（消除死锁/不变式违反）或拆解规格（缓解状态爆炸），再重跑 `check-tla-model.ts` | `check-tla-model.ts` 退出码 0 才算行为门禁通过；完整链为普通 V/G 失败链（见「普通 V/G 失败链」节） |
| #16 | TLA+ 规格含 `Next = []` 空下一步 / `\* TODO` 未实现分支 / 刻意遗漏需求关键状态 / 不变式与设计文档矛盾 | 先将问题记录为 R 定位线索；完成普通 V/G 失败链后，按 R 结论由 S-fix 重写 TLA+ 规格（补全状态分支、对齐需求/设计），再重跑 V→G | V 评审 `passed=false` + `check-tla-model.ts` 退出码 0；完整链为普通 V/G 失败链（见「普通 V/G 失败链」节） |
| #17 | TLC 发现违反，S 核查后确认规格忠实于需求/设计，但未回退修正需求/设计 | 回退到对应阶段：修正需求规格或设计文档 → 重写 TLA+ 规格 → 重跑 TLC | `check-tla-model.ts` 退出码 0（修正后重跑通过） |
| #18 | V/G 不通过后编排者直接分派 S 返工（无 R 报告作为 S-fix 输入） | 禁止直接分派 S；必须执行普通 V/G 失败链（见「普通 V/G 失败链」节） | `check-rootcause-report.ts` 退出码 0 + run-log R3 扩展（R+S-fix 一一对应） |
...
| `check-verifier-output.ts` | 1 | 评审未通过（schema / 方差 / 分数不达标） | #1 / #4 | 先走普通 V/G 失败链（见「普通 V/G 失败链」节），再按 R 结论由 S-fix 返工 |
| `check-verifier-output.ts` | 2 | 输入错误（JSON 缺失 / 路径错误） | #1 | 重新执行评审产出 JSON |
| `check-artifact-gate.ts` | 0 | 质量门通过 | — | 可发布 |
| `check-artifact-gate.ts` | 1 | 质量门未通过（覆盖率 / 测试状态不达标） | #3 / #6 / #7 | 先走普通 V/G 失败链（见「普通 V/G 失败链」节），再按 R 结论由 S-fix 返工 |
...
| `check-requirement-graph.ts` | 1 | 图谱校验失败（孤立 / 多根 / orphan / multiParent / 追溯违反 / blackHoles / miracles / deadModules / boundary 不完整） | #11 / #12 / #13 | 阶段 1 ingestion 图谱失败仅走 A-chunk/A-cross→G 专用收敛；其他普通失败先进入普通 V/G 失败链（见「普通 V/G 失败链」节），再按 R 结论补跑 A→G 收敛循环 |
...
**与 #16 的关系**：#16 是规格本身有缺陷（偏离需求/设计），#17 是规格忠实但需求/设计本身有缺陷——前者修规格，后者修需求/设计并回退。判定流程见 [tla-plus.md](tla-plus.md)「建模与需求/设计一致性」节。
...
- BDD↔TLA+ 不等价时必须走普通 V/G 失败链（见「普通 V/G 失败链」节），不得直接放行。
...
**关联**：
- spec §10.1 / bdd.md §8 / check-bdd-model.ts D4 等价性校验。
- bdd.md 第 3 项「TLA+ 等价性」。
...
**关联**：`check-archive-integrity.ts` 归档完整性清单
...
**回退动作**：V 按 R13 将对应子标准标记为 violation，`passed=false`，产出 `reworkHints` 交 R 形成定位线索；完成普通 V/G 失败链（见「普通 V/G 失败链」节）后才由 S-fix 返工；qualityLevel 不变（仍由 compositeScore 映射），仅 passed 增加单轴条件。
...
**完整返工循环**：普通 V/G 失败链（见「普通 V/G 失败链」节）。

### #48 子代理越界实施

**定义**：六角色中任一子代理执行其角色边界外的动作。边界见 [subagent-delegation.md](subagent-delegation.md)「强制约束」与「dispatch-matrix」§1：S 只产出（含 RTM 回填与 inputProvenance 签名）；V 只评审（产出 VerifierOutput JSON）；G 只跑门禁与回填证据；A 只产出 ingestion 中间产物；R 只产出根因/预防性审查报告。
