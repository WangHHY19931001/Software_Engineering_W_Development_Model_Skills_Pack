...
| `regression-detected` | 阶段 6/7（集成/系统测试） | L2+，回归测试失败；先完成完整普通失败链 `V/G 失败 → R → V 复审 RootCauseReport → G(check-rootcause-report exit 0) → S-fix → R3×3 → G(check-preventive-review exit 0) → V → G → CHECKPOINT`，再按 R 结论路由 | - |
