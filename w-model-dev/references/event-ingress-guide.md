...
| `regression-detected` | 阶段 6/7（集成/系统测试） | L2+，回归测试失败；先完成普通 V/G 失败链（hard-constraints），再按 R 结论路由 | - |
...
- 核实不通过（复现不出 / 相关测试未通过）→ **不得**按 reporter 的定性直接分类（不得直接判 `bug`、不得直接进阶段 5 修复）；应记录核实结果（含已尝试步骤与卡点）并**交人裁决**（主出口）**/ 转 R 定位**，run-log 留痕。**「转 R」不构成 `root-cause-locator.md` §2.7 R 入场门的旁路**：核实不通过本身即「无红信号」，故必须先由 S 按 §2.8 清单构造出达标红信号（§2.7 五件套）才能转 R 定位；造不出则按 §2.7「造不出红信号：停下置 blocked 三选一」处理，不得凭本节核实结果让 R 直接进入假设阶段。
