...
> 本小节补全非阶段门触发的工具类 CLI、code-health 门禁 CLI 与元门禁脚本，与 `w-model-dev/scripts/cli/` 目录 45 个 .ts
> 一一对应（26 个 check-* + 7 个 code-health 门禁 CLI + 12 个工具：ensure-codegraph-opsx 见 §5 / 其余见下表；其中 44 个为 exit-2 脚本，self-test.ts 为 exit 0/1 回归基线，与 conventions.md「= 44（26 个 check-* + 18 个工具 CLI，不含 self-test；18 = 7 个 code-health 门禁 CLI + 11 个工具 CLI）」口径互补）。
> **新增 / 改名门禁脚本时登记点为本表 + SKILL.md/AGENTS.md 计数句（由 checkScriptRegistry 与计数检查双向兜底）**——`check-docs-consistency.ts` 的 checkScriptRegistry
> 核对全部 45 个 cli 脚本名均出现于本文件，SKILL.md「N 个 .ts」/ AGENTS.md「N 个脚本」/ conventions.md 计数句由计数检查同步核对（漏登记即门禁失败，pre-push 第 14 项拦截）。
...
| check-samples-coverage        | 元门禁                   | samples 覆盖矩阵门禁（四条规则：fixture 均被 self-test 引用 / 引用路径无悬空 / 子目录均在 samples/README 矩阵声明 / exit-2 门禁均在 NEGATIVE-COVERAGE 登记会失败的负向案例——未登记 missing、证据悬空 dangling 均 exit 1）                                                                                                                                         | 仓库维护（pre-push 第 15 项），非项目阶段门                                        |
...
| self-test                     | 工具                     | 342 条样本回归基线（全部 check 逻辑通过/失败/输入错误三态）                                                                                                                                                                                                                                      | 仓库维护（pre-push 第 1 项），非项目阶段门                                         |
