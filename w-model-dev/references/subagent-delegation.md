...
**修正权**（与约束 #8『编排者最小化』、反模式 #10『编排者越权实施』互补）：O 不实施（agent 侧约束），但**用户**保留修正权——人在回路的最低标准 = 能在过程中间改产物而不用重跑一遍。凡只提供审计权（日志/面板/思维链展示）而无修正路径的产物设计视为不合格（见 [anti-patterns.md](anti-patterns.md) 反模式 #46）。
