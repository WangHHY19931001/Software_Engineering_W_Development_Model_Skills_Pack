...
> **编排者越权实施**（如 O 直接产出 / 评审 / 跑门禁并替代 G 回填）命中反模式 #10，回到当前阶段起点（见 [hard-constraints.md](hard-constraints.md) #10）。
...
>
> **子代理分派列**标注本阶段由哪些角色执行（O=编排者 / S=产出子代理 / V=评审子代理 / G=门禁子代理 / A=分析子代理 / R=根因定位子代理）。编排者不得越权实施（反模式 #10）。
...
> 🔴 **CHECKPOINT · 每个阶段门**：流程图中每个「评审」节点都是暂停点。Agent 必须按 [verifier-spec.md](verifier-spec.md) §8 提示词执行 LLM-as-a-Verifier 评审，调用 `npx tsx w-model-dev/scripts/cli/check-verifier-output.ts <output.json>` 防漂移，向用户展示「质量等级 / 各子标准分 / reworkHints」由用户确认放行或返工。**禁止越过评审节点自动推进**（见 [hard-constraints.md](hard-constraints.md) #1/#8）。
...
阶段 1–4 产出 TLA+ 规格与 BDD features，除 `check-verifier-output.ts` 外，G 子代理还须跑：
...
> 与 [hard-constraints.md](hard-constraints.md) 互引：以下是工作流执行中的高发陷阱，命中即回退到对应阶段起点。
