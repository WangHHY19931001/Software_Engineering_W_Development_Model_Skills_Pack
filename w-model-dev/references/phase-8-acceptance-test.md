...
| 6 | 文档完整性只检查文件存在 | 必须对照 [templates/](../templates/) 13 个模板逐一核验内容完整性 |
...
| 部分用例通过 | 通过项归档为「阶段性验收通过」；未通过项按 reject 路径进入上述普通 V/G 失败链（hard-constraints）；用户须显式选择「接受部分通过 + 缺陷追溯」或「整体 reject」 |
...
**回退决策树（仅供普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）完成后的 R 分类与用户 CHECKPOINT 决策使用，不是直接回退授权）**：
