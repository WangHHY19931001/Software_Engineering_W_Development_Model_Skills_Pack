...
> 🔴 **CHECKPOINT · 验收放行（项目级）**：验收测试执行完成后暂停。Agent 必须向用户逐条展示「项目级验收检查清单 12 项核验结果 / RTM 覆盖率 / 四级测试汇总 / 用户确认请求」，由用户在 RTM 签字列填入 `confirm` 或 `confirm-with-comments` 才能归档。RTM 覆盖率 < 100% 或四级测试任一未通过 → 一律不得请求用户确认，必须回对应阶段返工。
