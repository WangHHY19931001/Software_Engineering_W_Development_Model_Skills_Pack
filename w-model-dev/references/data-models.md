...
- `coverageStatus` 仅用于展示，门禁脚本从原始字段重算（见 [rtm-guide.md](rtm-guide.md) 覆盖率算法）；手工预填 `100%` 不被信任。`wm-status.ts` 与 `check-artifact-gate.ts` 共用 `logic/gate-logic.ts` 的 `computeRtmTraceCoverage(rows, phase)`，两者数值必然一致。
