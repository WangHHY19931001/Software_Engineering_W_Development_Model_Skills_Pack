...
- `coverageStatus` 仅用于展示，门禁脚本从原始字段重算（见 [rtm-guide.md](rtm-guide.md) 覆盖率算法）；手工预填 `100%` 不被信任。`wm-status.ts` 与 `check-artifact-gate.ts` 共用 `logic/gate-logic.ts` 的 `computeRtmTraceCoverage(rows, phase)`，**在同一 `phase` 下**两者数值必然一致（`wm-status` 取 `project.status` 对应阶段，聚合门取 `--phase`、缺省 8；阶段输入不同则字段集不同）。

**测试执行证据 `executionSummary.<layer>.evidence`（M07 / D-2，2026-09-15）**：`definitions.testSummary` 新增**可选**对象 `evidence`（`rtm.schema.json`；`required:["command","exitCode","observedAt"]`，`additionalProperties:false`），把该层 `passed/failed/pending` 摘要绑定到真实运行——`command`（非空、禁 `;` `&` `|` `<` `>` 与换行）、`exitCode`（非负整数）、`observedAt`（UTC ISO-8601 毫秒），可选 `rawOutputPath`（项目根相对路径，与 `rawOutputSha256` 成对）与 `rawOutputSha256`（64 位小写十六进制）。**RTM 行字段、追溯关系与覆盖率语义均未变动**（D-2 硬约束：只新增可选字段）。门禁 `check-artifact-gate.ts --phase=N`（阶段 5~~8）按四条规则校验：E1 配对、E2 哈希核验（相对项目根解析、文件须存在、SHA-256 须相符）、E3 结果一致性（`failed=0 && pending=0` ⇒ `exitCode=0`；`failed>0` ⇒ `exitCode≥1`；`failed=0 && pending>0` 不约束）、E4 当前阶段层 `total>0` 的 `evidence` 存在性。**严格语义（无时间戳豁免）**：阶段 5~~8 中当前阶段层 `total>0` 而缺合法 `evidence` **始终**是阻断错误（E4），与 `lastUpdated` 早晚无关——`lastUpdated` 只是 RTM 元数据，不参与信任判定；历史上曾存在的 `M07_TEST_EVIDENCE_CUTOFF` / `LEGACY_TEST_EVIDENCE` 时间戳吸收路径**已删除**（旧数据须重跑真实命令、保存原始输出与 SHA-256，并经 `wm-verify-evidence-source` producer 记录当前 HEAD / source bundle / 运行身份后迁移，不得靠回填旧时间戳迁移）。严格模式下 `testEvidence.legacy` **恒为 0**，含义是「没有时间戳豁免」，不是「历史记录已被证明有效」。`GATE_JSON` 摘要字段 `testEvidence` 为 8 键计数对象（`checked` / `withEvidence` / `missing` / `legacy` / `e1`~~`e4`）——**E4 只作用于当前阶段必须存在的层（阶段 5~~8），E1~~E3 只要某层携带 `evidence` 即生效、与阶段无关**；注意 `testEvidence.legacy`（数值）与顶层 `legacy`（非阻断诊断数组，仅非空时出现）**同名不同型**。完整形状见 `rtm-guide.md`「测试执行证据（M07）」节。**边界**：与 run-log `revertEvidence`（R10）**载体不同、绑定对象不同、不得互相替代**——RTM 证据绑定阶段级测试运行，run-log 证据绑定 S-fix 复现测试的回滚证伪（详见 `rtm-guide.md`「测试执行证据（M07）」节）。
...
迁移步骤：备份 `cp .w-model/rtm.json .w-model/rtm.json.bak.<ts>` → 执行迁移逐条更新 status → 跑 `check-artifact-gate.ts [project-dir] --phase=<N>`（N=当前阶段）退出码 0 才算成功；失败则回滚 `.bak.<ts>`。阶段 5-8 另须 `--scope=<change-scope.json>`。
...
## S18 票据内容模型（tickets.md）

`check-artifact-gate.ts --tickets=<path>` 在 phase 5–8 对票据块执行 S18 内容门禁。票据中的反引号 span 以 owner/member 调用头（或无 owner 函数名）作为 canonical symbol key：定义 `Foo.run(job): Result` 只登记 `Foo.run`，参数名、返回类型、普通共享 token 和嵌套泛型不参与匹配；因此 `Other.call(job)` 必须报告完整未定义符号头，而 `Foo.run(task)` 可匹配。点号与调用空格会被规范化，不同 owner 的同名 method 仍不相等，重复定义幂等处理；无 owner 调用按函数名精确匹配。契约标记词仅在与 span 声明式邻接时定义符号，否定/旁述不构成定义；`What to build` 行的空括号调用式不构成定义。
...
| `coverage`                      | `coverage.schema.json`                      | CoverageAnalysis           | additionalProperties:false；4 张覆盖矩阵（stakeholder/scenario/requirementType/crossCuts）+ metrics.* 数值无 schema 边界，以消费方 `===100` 判定为准（coverage-logic C8）              | coverage-logic.ts                                                                     |
...
| `signature-chain`               | `signature-chain.schema.json`               | SignatureChainEntry        | additionalProperties:false；inputProvenance 来源证明；role / inputProvenance.sourceRole enum（无 actorRole 字段）                                                                      | （暂未接入 validateBySchema，经 readJsonlOrExit 标签间接使用）                        |
...
2. **`additionalProperties:false` 全层级**：顶层 + 嵌套对象均禁止未知字段，防字段漂移（如 VerifierOutput 顶层多 `unknownField` → schema 拒绝）。唯一开集例外：`code-tla-manifest` 的 manifest/graph/rtm 兼容层（含其数组 items）声明 `additionalProperties:true`——该层允许扩展字段，需兼容完整 TlaManifest/Graph/Rtm 类型（schema description 自述）；`tla-manifest.checkRounds[].` 元素无此豁免（F-G4-13 已收紧为 false）。
