...
> 对应 spec §5.5（`docs/superpowers/specs/2026-07-24-root-cause-locator-and-fixer-roles-design.md`） run-log 新增动作 + §9.9（`docs/superpowers/specs/2026-07-24-root-cause-locator-and-fixer-roles-design.md`） Token 预算扩展。普通返工循环固定为 普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节），以下为该链中各动作的 token 计量约定。
