...
| 泛化词须追问 | 用户仅说「继续」「OK」「确认」「同意」「好的」「yes」等泛化词时，O **不得**直接放行，须追问「请确认具体技术决策」直至用户给出含具体名词（技术方案名/模块名/接口名/数据结构名）的决策 | 泛化词命中 `check-checkpoint.ts` R2 黑名单 → exitCode=1 |
...
> 四个闭环校验脚本（`check-budget.ts` / `check-run-log.ts` / `check-maturity.ts` / `check-checkpoint.ts`）在每个阶段门由 G 子代理执行，强制校验 budget / run-log / maturity / checkpoint 四套机制的完整性。规则表见修正设计 [§5.1-5.4](../../docs/superpowers/specs/2026-07-23-w-model-dev-correction-design.md)；SSoT [§10C.7](../../docs/skill-design-document_SSoT.md) / [§10D.7](../../docs/skill-design-document_SSoT.md) / [§10E](../../docs/skill-design-document_SSoT.md) 为权威定义。本节仅约定调用时机与校验内容摘要；异常处置（预算超限、kill switch、日志损坏、成熟度降级等）见上方「成本预算与运行日志」「成熟度与 CHECKPOINT 放行」两节，不在此重复。
...
| `check-checkpoint.ts`（§5.4） | R1 acknowledgedDecisions 非空 · R2 决策内容具体（泛化词黑名单）· R3 用户确认存在 · R4 决策与阶段匹配 |

### 脚本间依赖

`check-run-log.ts` 交叉校验 `gate-logs/` 存档与 run-log `gateExitCode` 一致（SSoT §10E.3）；`check-checkpoint.ts` 依赖 run-log 中 checkpoint 记录；`check-budget.ts` / `check-maturity.ts` 交叉 run-log 统计返工/成功阶段数。四脚本须在阶段门一并跑完，不得跳过；CLI 用法与退出码语义（0 通过 / 1 校验失败 / 2 输入错误）见修正设计 §5.1-5.4。
