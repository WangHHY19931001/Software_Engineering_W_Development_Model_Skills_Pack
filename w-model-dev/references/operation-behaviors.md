...
## 代码健康治理中的操作行为（`/wm code-health`）

八条行为在代码健康治理中的强制落点（详见 [code-health-governance.md](code-health-governance.md)）：

- **Surface Assumptions**：候选的 `confidence.uncertainties` 与 `review.unresolvedQuestions` 必须显式列出；material ambiguity fail-closed。
- **Manage Confusion Actively**：环境 `unavailable` / `not_run` 不得当作 observed；展示为 `unexercisedScenarios` 而非猜测。
- **Push Back When Warranted**：无人类 approval 或 scope 漂移时拒绝执行；不得以「看起来是死代码」放行。
- **Enforce Simplicity**：抽象要有量化维护收益与 ≥2 个稳定生产调用点；「少几行 diff」不构成理由。
- **Maintain Scope Discipline**：只改人类批准的精确 scope；scope 外任何变更即 exit 1。
- **Verify, Don't Assume**：以真实 `CommandEvidence`（数字 exitCode + observed + raw output hash）与真实 pre/post suite 身份证据为准，不估算。
- **Choose Highest Seam**：Phase 3 复用既有测试面而非新建 seam；Phase 4 结构化证据优先于自由文本视图。
- **Structure Over Persuasion**：授权写进 ledger + HEAD-tracked 事实源 + 人类 approval 结构，而不是靠提示词约束。
