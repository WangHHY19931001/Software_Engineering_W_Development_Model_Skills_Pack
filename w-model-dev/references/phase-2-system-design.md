...
## 系统设计算法
  ...
  1. 系统架构建模
     ...
     ├─ 失败: 架构图无数据流标注 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论补全组件间数据流向（FM-SD-01）
     └─ 成功: 子系统清单与模块划分候选对应
  2. 技术选型与 ADR
     ├─ 按技术选型决策矩阵 5 维度评分（适用性/成熟度/可维护性/引入成本/风险敞口）
     ├─ 架构决策记录 ADR 写入 system-architecture.md §5
     ├─ 失败: 选型无评分依据 / ADR 无上下文-后果 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论补步骤 2（FM-SD-02）
     └─ 成功: 选型理由成立，主模板 §2 技术选型表填实
  3. 模块划分与部署架构
     ├─ 基于子系统清单，产出主模板 §3 模块划分表（模块 ID 与子系统清单对应）
     ├─ 产出主模板 §4 部署架构
     ├─ 失败: 模块循环依赖 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论重新划分（FM-SD-03）
     ├─ 失败: 子系统清单与模块划分不对应 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论处理
     └─ 成功: 模块划分无环且与子系统清单一致
  4. 系统上下文与术语建模
  ...
  5. UML 系统级建模
     ...
     ├─ 失败: 图与主模板 §1/§3 不对应 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论对齐步骤 5（FM-SD-04）
     └─ 成功: 四图产出，mermaid 块配平
  6. 追踪矩阵与行为规格引用
     ...
     ├─ 主模板 §7/§8 引用块指向上述独立文件
     ├─ 失败: 追踪矩阵字段与步骤 1/3 不一致 → 记为 R 定位线索；普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论对齐步骤 6（FM-SD-05）
...
**ADR 入选门槛（三问，准入前置）**：5 维度评分决定「选哪个技术」，本门槛决定「**要不要为它写 ADR**」——三问全为真才立档：① **难逆（Hard to reverse）**——日后改变决定的代价是有意义的；② **无上下文会困惑（Surprising without context）**——未来的读者看代码会问「当初为什么这么做」；③ **真取舍（The result of a real trade-off）**——确实存在真实备选，且因具体理由选了其中一个。三问不齐（易逆 / 不令人困惑 / 无真实备选）→ **不写 ADR**，把决定记入设计文档即可：易逆的以后直接改；不困惑的没人会问；没有备选的除「我们做了显而易见的事」外无可记录。本门槛不替代上面的评分矩阵，也不改变步骤 2 的产出（ADR 仍写入 `system-architecture.md §5`）。
...
| 模块循环依赖 | `npx -y madge --circular --extensions ts,js <module-root>` 或 `npx -y dependency-cruiser -c .dependency-cruiser.cjs <src>` 退出码 ≠ 0 | 列出环路径，普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论重新拆分模块边界（引入接口层 / 倒置依赖），禁止带环放行 |
| C4 组件图缺数据流标注 | 图中无 `-.->` 或 `>>` 数据流箭头 | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论补全架构图的组件间数据流向（输入/输出/同步异步） |
| 强一致数据被拆到多服务 | 跨服务强一致操作（原可单库事务） | 聚合数据到单一服务，或显式采用柔性事务（TCC/SAGA）并登记失败路径 |
| 分区/网络异常下的可用性声明 | 系统无分区处理策略 | 按 CAP 三选一显式声明取舍（AP/CP），写入 ADR |

执行顺序：先跑检测命令（以退出码为准），再将失败记录为 R 定位线索；普通 V/G、评审或阶段门失败必须走普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节），S-fix 后重新检测，确保闭环。

## 失败模式矩阵

| 编号 | 失败模式 | 检测信号 | 处置 |
|---|---|---|---|
| FM-SD-01 | 架构图缺数据流标注 | system-architecture.md 组件图无 `-.->`/`>>` 数据流箭头 | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论补步骤 1 的数据流向 |
| FM-SD-02 | 选型无评分依据 / ADR 缺上下文后果 | 技术选型表无 5 维度评分；ADR 缺 context/consequences | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论补步骤 2 的评分与 ADR 结构 |
| FM-SD-03 | 模块循环依赖 | 模块划分 DFS 三色染色检测到环 | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论重新划分步骤 3 的边界 |
| FM-SD-04 | UML 建模与架构/模块划分脱节 | uml-modeling.md 图与主模板 §1/§3 不对应 | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论对齐步骤 5 UML 建模 |
| FM-SD-05 | 追踪矩阵字段不一致 | traceability-matrix.md 与主模板 §3/phase1 追踪矩阵不一致 | 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints）后，按 R 结论对齐步骤 6 追踪矩阵字段 |

> 注：FM-SD-06（越过阶段边界落接口/类级）为越界检测信号，见禁止行为 #8 与返工路径，不单列于上表。

> **门禁强制（FM-SD-02 的 ADR 半边）**：`check-artifact-gate.ts --phase=2 --spec-dir=<dir>` 的 `adr` 桶逐行校验 `*-system-architecture.md` §5 表——`决策` / `上下文` / `后果` 三列均不得为空（`{{...}}` 占位符、`—`、`-` 视为未填），故此半边不再只靠 V 评审。**另外半边（选型无评分依据）仍为 V 评审判据**，脚本不判评分质量。**判据边界**：表存在才校验，表为空或缺节不报——是否该立 ADR 属入选三问的判断型准入，脚本判不了。
...
G 子代理跑 [`check-bdd-model.ts`](../scripts/cli/check-bdd-model.ts) `--phase=2 --require-tla-equivalence --tla-manifest=.w-model/tla-manifest.json --graph=.w-model/ingestion/graph.json` 校验 D1-D8；phase>=2 缺 --graph 为 ARG_INVALID/exit 2（进入 D1-D8 前即拒绝；graph 为 D8 的数据源）。
...
评审通过 → 进入阶段 3（概要设计）。
评审不通过 → 必须执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）；只有用户在 CHECKPOINT 确认后，才可按 R 的 upstreamDefect 结论回到系统设计对应步骤。

### 普通 V/G 失败链

普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）

该链适用于阶段 2 的评审、测试设计和门禁失败；不得按 reworkHints 直接返工或自动回到上游阶段。
...
- 架构不清晰 → 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论补充功能描述中的分层 / 部署 / 数据流
- 模块划分有循环依赖 → 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论重新划分模块边界
- 系统测试缺性能 / 安全基线 → 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论补充并行任务中的 TC-DES-008/009
- 技术选型无原则依据 → 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论按「技术选型决策矩阵」5 维度评分补全理由
- 端到端用例缺失 → 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论补全并行任务中的 TC-DES-007
- 架构图缺数据流（FM-SD-01）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论回步骤 1 补全
- 选型无依据（FM-SD-02）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论回步骤 2 补全评分/ADR
- 循环依赖（FM-SD-03）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论回步骤 3 重新划分
- UML 脱节（FM-SD-04）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论回步骤 5 对齐
- 追踪矩阵不一致（FM-SD-05）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论回步骤 6 对齐
- 越界落接口/类级（FM-SD-06）→ 普通 V/G 失败先执行普通 V/G 失败链（hard-constraints.md「普通 V/G 失败链」节）后，按 R 结论移除越界内容，并在用户 CHECKPOINT 后移交阶段 3/4
