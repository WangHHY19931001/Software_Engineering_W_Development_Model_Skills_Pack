...
节点 id 格式 `<TYPE>-<NNN>` 全局唯一。**唯一性由 R16 机器强制**（不只是书写约定）：`nodes[].id` 重复即 `check-requirement-graph.ts` 退出码 1。判据在构建 id 集合**之前**执行——否则重复 id 会被 `Set` 静默吞掉，连通性/孤立/父唯一/环/信息流全部在「多节点合并为一个判定单元」的语义上计算，两个节点各自的问题互相抵消而门禁仍判通过。
...
| governs | 治理类子系统→被治理子系统 | 横切治理（如 S08 governs 多个子系统） | ≥0；源须为 `governance===true` 的治理类子系统（如 S08）；不依附层级树（见 §7） |
| collaborates-with | 节点↔节点 | 对等协作（单条边语义双向） | ≥0；禁止指向不存在节点；不依附层级树（见 §7） |
| derives | S11→派生产物 | 派生规格（如 TLA+ spec derives 自设计节点） | ≥0；源须为 `derivationProduct===true` 的派生规格节点（如 S11）；不依附层级树（见 §7） |
...
| 7 | 派生层 | `derives` | 派生规格（S11→派生产物） | 源须为 `derivationProduct===true` 的派生规格节点（如 S11）（marker 定义见 data-models.md）；目标须为派生产物且存在；不依附层级树 |
...
- **边界节点**：EXT-IN（源）/ EXT-OUT（汇）显式化系统边界（DFD terminator），不参与 parent 系统层级树，参与连通性与信息流。阶段 1 起须各 ≥1。
...
| 维度 | 校验什么 | 产物 | 门禁脚本 |
|---|---|---|---|
| 结构 + 信息流（本文件） | 节点归属系统层级树 + 追溯完整 + 无黑洞/奇迹/死模块 | `graph.json` | `check-requirement-graph.ts` |
...
TLA+ 层次化建模（L1–L6）、文件头规范、SANY/TLC 校验顺序、拆解阈值（>1k 考虑拆 / >1w 必须拆）详见 [tla-plus.md](./tla-plus.md) 与 `docs/tla-plus-modeling-design.md`。

## 边数下限与语义来源占比

> 图谱规模阈值须有边数下限与语义来源占比指导，避免靠补丁达成规模。
