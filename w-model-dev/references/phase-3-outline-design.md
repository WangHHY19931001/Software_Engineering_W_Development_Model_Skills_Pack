...
- 接口签名无类型约束 → 普通 V/G 失败走完整普通失败链后，按 R 结论回到接口定义补全参数与返回值类型
- 越界深入类 / 方法内部 → 普通 V/G 失败走完整普通失败链后，按 R 结论移除越界内容并将类 / 方法级设计移交阶段 4
- 接口契约缺字段/错误码（FM-OD-01）→ 普通 V/G 失败走完整普通失败链后，按 R 结论回步骤 1 补全
- 字段语义模糊（FM-OD-02）→ 普通 V/G 失败走完整普通失败链后，按 R 结论回步骤 3 补全映射
- 循环依赖（FM-OD-03）→ 普通 V/G 失败走完整普通失败链后，按 R 结论回步骤 2 重新划分
- UML 脱节（FM-OD-04）→ 普通 V/G 失败走完整普通失败链后，按 R 结论回步骤 5 对齐
- 追踪矩阵不一致（FM-OD-05）→ 普通 V/G 失败走完整普通失败链后，按 R 结论回步骤 6 对齐
