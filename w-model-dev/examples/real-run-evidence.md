...
> 本文件是 examples/ 中唯一的**历史真实证据快照**：以下命令在本仓库实际执行（2026-08-13，41.9.0），输出为当日真实截取（含退出码与 JSON 摘要），现按版本与日期锁定，不滚动改写为当前版本。对话类示例（coding / requirement-analysis / system-design / test-execution）为「伪示例」，仅供 LLM 行为对齐，其字段不代表真实输出。
>
> **比较边界**：本快照中的 `fixtureCount=272`、`referencedFiles=234`、`referencedDirs=15` 只描述 41.9.0 当日资产。42.2.0 验收记录中的 `282/244/15` 来自另一版本、另一轮真实运行；两组数值不得用于推导通过率变化、回归或资产增减结论，也不得把 42.2.0 数值覆盖进本历史块。需要判断当前状态时，必须在当前 HEAD 重跑命令并单独记录版本、日期、HEAD 与退出码。
>
...
- 引用本文件时必须同时标注“41.9.0 历史快照”；它证明 2026-08-13 的运行，不证明当前 HEAD。
- 若脚本输出契约演进，应在新版本证据记录中重跑并登记，不得就地改写本锁定快照或拿历史计数冒充当前测量。
- 对话类示例的退出码、JSON 前缀和错误分类应以当前脚本契约为准；字段变化须由当次真实命令验证。
