...
- **第三十一轮 Schema 自描述 + 脱敏 + audit**：19 份 schema 全量字段补 description（仅注释性关键字，校验行为不变）/ 反模式 #43（状态文件/日志不得写入敏感凭据，敏感配置统一环境变量注入）/ pre-push 新增 npm audit warn-only（第 12 项）。详见 SSoT §3.4.27。
- **第 30.1 轮 security-scan 内容指纹 v2 + 签名链 R8 语义**：安全扫描指纹从位置敏感改为内容敏感（baseline v2，行号漂移免疫）+ `--regenerate` 重生成；check-signature-chain R8 仅在真实项目根（.w-model/project.json）启用。详见 SSoT §3.4.28。
- **第三十二轮 错误结构全量归一化 + run-log R6 契约迁移**：新增 `scripts/lib/cli-error.ts`（6 类错误码 ARG_INVALID/FILE_NOT_FOUND/FILE_PARSE/FILE_READ/STRUCTURE_INVALID/UNEXPECTED + `CliError` + `exitWithError`，exit 2 统一输出人类消息 stderr + `ERROR_JSON` stdout，遵循 §10E E.1 exitCode 强一致；`process.exitCode` 自然退出防 stdout 截断）；29 个脚本（23 check-*.ts + 5 工具 + read-json-or-exit）exit 2 归一化；`extractExitCode`/`buildGateLogKeys` 迁入 `run-log-logic.ts` 纯逻辑层（GATE_JSON_PATTERNS 25→26 追加 ERROR_JSON，exit 2 存档纳入 R6 交叉校验）。详见 SSoT §3.4.30。
- **第三十六轮 冰山扫掠深度分析机制**：基于冰山理论新增 R-iceberg 冰山扫掠机制——S-fix 后（ICEBERG-A）与阶段门放行前（ICEBERG-B）以已发现/已修复问题为线索主动深挖隐藏问题（三维度×六类别），直到 `newFindings=[]` 或达 maxIcebergRounds=5。新增 `iceberg-sweep.schema.json` + `iceberg-sweep-logic.ts`（R1-R8）+ `check-iceberg-sweep.ts` + `references/iceberg-sweep-guide.md` + 反模式 #44（43→44）；run-log action 枚举 25→27（+iceberg-sweep/iceberg-review）；self-test 基线 213→217。版本号三处 36.0.0。详见 SSoT §3.4.34。
- **第 37 轮 Phase 1 设计级别增强**：阶段 1 需求规格产出升级——主模板 + 6 独立子模板（system-context/glossary/traceability-matrix/behavior-spec/discipline-dod/uml-modeling），主规格引用块串联，不再内联 feature 集（行为规格由 bdd .feature 文件 + bdd-manifest.json 承接）；`check-requirement-graph.ts` 新增 R7/R8（--spec-dir），`check-artifact-gate.ts --phase=1` 新增引用块/SSOT/DoD 校验。反模式总数 44 不变（仅 phase-1-requirements.md 内 FM-3D-08/09 + 禁止行为 #13/#14）。详见 SSoT §3.4.35。
...
npm run self-test                           # 225 条样本回归基线（19 Verifier + 19 Gate + 28 Graph + 4 SpecEnhance + 4 SpecStructure + 10 Coverage + 7 Exemption + 14 TLA + 5 Budget + 13 RunLog + 3 Maturity + 2 Checkpoint + 5 Code-TLA + 12 RootCause + 16 Schema + 1 Metadata + 10 BDD + 15 SignatureChain + 4 ArchiveIntegrity + 2 PreventiveReview + 4 IcebergSweep + 2 TlaBddSync + 3 RoleDispatch + 3 StateMachine + 4 CodegraphQuery + 3 OpsxArtifact + 3 OpenspecArchive + 5 UAT_PATH_MAPPING + 5 DESIGN_CONTRACT），退出码 0/1
...
# 手动跑推送前门禁（不实际推送，12 项门禁检查）
npm run prepush

npm run lint:security              # 跑 eslint-plugin-security + baseline 比对，退出码 0/1（devDep：eslint + @typescript-eslint/* + eslint-plugin-security）；npx tsx w-model-dev/scripts/security-scan.ts --regenerate 可重生成 baseline
...
| check-signature-chain.ts | 角色链式签名门禁（R1-R10：Schema 完整性/签名者角色/时序/输入来源证明/跨阶段消费者校验；R8 仅真实项目根启用；CLI `<signature-chain.jsonl>`） | 全阶段 | 0=通过，1=校验失败，2=输入错误 |
...
| check-iceberg-sweep.ts | IcebergSweepReport 校验（R1 schema 前置 / R2 reportId 格式 / R3 phase 一致 / R4 triggerType 合法 / R5 round 边界 1-5 / R6 去重 / R7 可证伪 / R8 passed 一致，反模式 #44；CLI `<report.json>` 或 `--auto-trigger --run-log=<path>`） | 1-8（S-fix 后 / 阶段门前） | 0=通过，1=校验失败，2=输入错误 |
...
| self-test.ts | 回归基线（225 条样本：19 Verifier + 19 Gate + 28 Graph + 4 SpecEnhance + 4 SpecStructure + 10 Coverage + 7 Exemption + 14 TLA + 5 Budget + 13 RunLog + 3 Maturity + 2 Checkpoint + 5 Code-TLA + 12 RootCause + 16 Schema + 1 Metadata + 10 BDD + 15 SignatureChain + 4 ArchiveIntegrity + 2 PreventiveReview + 4 IcebergSweep + 2 TlaBddSync + 3 RoleDispatch + 3 StateMachine + 4 CodegraphQuery + 3 OpsxArtifact + 3 OpenspecArchive + 5 UAT_PATH_MAPPING + 5 DESIGN_CONTRACT）；vitest 476 条（含 graph-logic R1-R6 + coverage-logic C1-C10 + exemption-logic E1-E8 + signature-chain R1-R10 + archive-integrity + preventive-review + iceberg-logic + tla-bdd-sync + plan-chunks + design-contract-logic + wm-status-logic + metrics-report-logic + wm-status CLI + metrics-report CLI + cli-error，34 test files） | - | 0=通过，1=失败 |
