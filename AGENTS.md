...
| `w-model-dev/schemas/` | JSON Schema (draft-07) 文件（21 份，全字段 description 自描述） | logic 层 schema 校验时自动加载；新增 .w-model/*.json 字段必先改 schema |
...
门禁脚本测试：
- `w-model-dev/scripts/__tests__/`：门禁脚本单元测试（vitest，50 个 .test.ts / 809 条）
...
| self-test.ts | 回归基线（256 条样本）；vitest 809 条（50 test files） | - | 0=通过，1=失败 |
