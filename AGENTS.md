...
- **错误结构全量归一化**：全仓 31 个脚本 exit 2 统一输出结构化错误——人类消息 `✗ [CATEGORY] ...` 走 stderr、`ERROR_JSON {...}` 摘要走 stdout（`lib/cli-error.ts`，6 类错误码）。详见 SSoT §3.4.30（全量归一化）+ §3.4.42（CliError rule/field 扩展，当前定义）。
...
门禁脚本测试：
- `w-model-dev/scripts/__tests__/`：门禁脚本单元测试（vitest，35 个 .test.ts / 576 条）
...
npm run self-test                           # 254 条样本回归基线，退出码 0/1
...
| self-test.ts | 回归基线（254 条样本）；vitest 576 条（35 test files） | - | 0=通过，1=失败 |
