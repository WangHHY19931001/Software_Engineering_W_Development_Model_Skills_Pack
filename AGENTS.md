...
- **错误结构全量归一化**：全仓 31 个脚本 exit 2 统一输出结构化错误——人类消息 `✗ [CATEGORY] ...` 走 stderr、`ERROR_JSON {...}` 摘要走 stdout（`lib/cli-error.ts`，6 类错误码）。详见 SSoT §3.4.30。
...
| `w-model-dev/scripts/` | 自包含门禁脚本（依赖 `tsx` runtime + devDeps：ajv / eslint-plugin-security，需 `npm install` 一次）：`w-model-dev/scripts/logic/gate-logic.ts` + `w-model-dev/scripts/cli/check-artifact-gate.ts`（工件质量门，含 TLA+ 资产 + SD→codeModule 终检 + RTM coverageStatus 一致性 + NFR 双字段校验 + 阶段级 `--phase=N`）/ `w-model-dev/scripts/logic/verifier-logic.ts` + `w-model-dev/scripts/cli/check-verifier-output.ts`（Verifier 校验，R13 单轴下限，支持 `--self-as-verifier --s-output=<path>`）/ `w-model-dev/scripts/logic/graph-logic.ts` + `w-model-dev/scripts/cli/check-requirement-graph.ts`（阶段 1–4 图谱结构门禁 + 信息流校验 + 边数下限 + 语义来源占比 + 阶段设计级 R7-R14）/ `w-model-dev/scripts/logic/tla-logic.ts` + `w-model-dev/scripts/cli/check-tla-model.ts`（阶段 1–4 TLA+ 行为门禁：SANY 语法 + TLC 模型检查，**已移除 `--skip-tlc`**）/ `w-model-dev/scripts/logic/code-tla-logic.ts` + `w-model-dev/scripts/cli/check-code-tla-consistency.ts`（阶段 5 代码-TLA+ 一致性回归：四维度）/ `w-model-dev/scripts/logic/budget-logic.ts` + `w-model-dev/scripts/cli/check-budget.ts`（Budget 门禁 R1-R5）/ `w-model-dev/scripts/logic/run-log-logic.ts` + `w-model-dev/scripts/cli/check-run-log.ts`（Run-log 门禁 R1-R7 + R3 预防性审查记录校验）/ `w-model-dev/scripts/logic/maturity-logic.ts` + `w-model-dev/scripts/cli/check-maturity.ts`（Maturity 门禁 R1-R5）/ `w-model-dev/scripts/logic/checkpoint-logic.ts` + `w-model-dev/scripts/cli/check-checkpoint.ts`（Checkpoint 门禁 R1-R5 + 拒绝代签）/ `w-model-dev/scripts/logic/root-cause-logic.ts` + `w-model-dev/scripts/cli/check-rootcause-report.ts`（RootCauseReport 校验 R1-R10）/ `w-model-dev/scripts/logic/signature-chain-logic.ts` + `w-model-dev/scripts/cli/check-signature-chain.ts`（签名链门禁 R1-R10 + 跨阶段消费者校验）/ `w-model-dev/scripts/logic/archive-integrity-logic.ts` + `w-model-dev/scripts/cli/check-archive-integrity.ts`（归档完整性校验）/ `w-model-dev/scripts/logic/preventive-review-logic.ts` + `w-model-dev/scripts/cli/check-preventive-review.ts`（R3 预防性审查三份报告校验）/ `w-model-dev/scripts/logic/iceberg-sweep-logic.ts` + `w-model-dev/scripts/cli/check-iceberg-sweep.ts`（IcebergSweepReport 校验 R1-R8）/ `w-model-dev/scripts/logic/tla-bdd-sync-logic.ts` + `w-model-dev/scripts/cli/check-tla-bdd-sync.ts`（TLA+/BDD 自动化同步校验）/ `w-model-dev/scripts/logic/role-dispatch-logic.ts` + `w-model-dev/scripts/cli/check-role-dispatch.ts`（角色分派完整性校验）/ `w-model-dev/scripts/cli/check-state-machine-consistency.ts`（设计文档↔代码状态机一致性）/ `w-model-dev/scripts/logic/design-contract-logic.ts` + `w-model-dev/scripts/cli/check-design-contract-consistency.ts`（设计契约一致性 D1-D4）/ `w-model-dev/scripts/logic/coverage-logic.ts` + `w-model-dev/scripts/cli/check-requirement-coverage.ts`（需求覆盖分析 C1-C10）/ `w-model-dev/scripts/logic/exemption-logic.ts` + `w-model-dev/scripts/cli/check-exemption.ts`（豁免审批 E1-E8）/ `w-model-dev/scripts/cli/ensure-codegraph-opsx.ts`（codegraph + OpenSpec 依赖三层检测+自动安装）/ `w-model-dev/scripts/cli/check-codegraph-queries.ts`（反模式 #38）/ `w-model-dev/scripts/cli/check-opsx-artifacts.ts`（反模式 #39/#40）/ `w-model-dev/scripts/cli/check-openspec-archive.ts`（opsx:archive 归档完整性）/ `w-model-dev/scripts/logic/schema-loader.ts`（ajv 单例 + schemas/ 自动加载）/ `w-model-dev/scripts/cli/security-scan.ts`（eslint-plugin-security 扫描 + baseline v2 内容敏感指纹豁免）/ `w-model-dev/scripts/logic/plan-chunks.ts`（ingestion 分块策略）/ `w-model-dev/scripts/logic/wm-status-logic.ts` + `w-model-dev/scripts/cli/wm-status.ts`（状态快照，只读）/ `w-model-dev/scripts/logic/metrics-report-logic.ts` + `w-model-dev/scripts/cli/metrics-report.ts`（流程度量，只读）/ `w-model-dev/scripts/cli/self-test.ts`（回归基线，252 条样本）/ `lib/read-json-or-exit.ts`（CLI 层 JSON/JSONL 读取工具）/ `lib/cli-error.ts`（exit 2 错误结构统一：6 类错误码）/ `lib/safe-json.ts`（JSON 原型污染防御）/ `__tests__/`（vitest 单元测试，35 个 .test.ts / 571 条 + README.md coverage 矩阵） | Agent 在阶段门 / 质量门 / 图谱门禁 / TLA+ 行为门禁 / 代码-TLA+ 一致性回归 / 签名链 / 归档完整性 / R3 预防性审查 / TLA+/BDD 同步 / 角色分派 / 状态机一致性 / 冰山扫掠检查点直接 `npx tsx` 执行 |
...
| `docs/` | 设计文档统一存放（SSoT / 集成设计 / 安装指南）；`docs/superpowers/`（plans/ + specs/）为内部规划目录，不参与门禁、非面向用户 | 修改设计先改 SSoT，再改 `w-model-dev/` 资产 |
| `eval/` | 外部工具（darwin-skill）评估产物归档 | 不属技能包，Agent 一般无需读取 |
| `.githooks/pre-push` | **本地 CI**：`git push` 时自动跑 15 项门禁（self-test + 门禁脚本退出码 + vitest 全量 + security-scan + npm audit（high 以上阻断；网络不可达或 registry 不支持 audit endpoint 自动跳过）+ samples 覆盖矩阵），任一不符即中止推送；替代远程 CI（仓库无 `.github/workflows/`，历史原因见 CHANGELOG）；平台补装见 `.githooks/ensure-platform-deps.sh` | 修改 `w-model-dev/scripts/**` / `package.json` / `.githooks/pre-push` / `.githooks/ensure-platform-deps.sh` 后会触发；Git Bash 与 WSL 下均正常执行门禁，仅纯 cmd/PowerShell 放行 |

门禁脚本测试：
- `w-model-dev/scripts/__tests__/`：门禁脚本单元测试（vitest，35 个 .test.ts / 571 条）
...
npm run self-test                           # 252 条样本回归基线，退出码 0/1
...
# 手动跑推送前门禁（不实际推送，15 项门禁检查；Windows 请用 Git Bash，WSL 可直接跑）
npm run prepush
...
- **回归测试强制钩子**（约束 #14）：任何 agent 改动代码后必须跑回归测试（第 39 轮 P1 批新增）；详见 [w-model-dev/references/phase-5-coding.md](./w-model-dev/references/phase-5-coding.md)「增量集成纪律」节。

## 7. 修复记录

- **四源吸收（APoSD《软件设计哲学》/ 凤凰架构 / GoF 设计模式 / 失控）**（2026-08-10）：P0/P1/P2 三批落地（41.0.0/41.1.0/41.2.0）——新增 references/four-source-absorption.md（吸收决策记录）+ design-patterns-catalog.md（GoF 23 模式目录）；设计质量判据（组 X 复杂度症状/信息泄露/复杂三症状/方案权衡列）、架构决策框架（CAP/微服务粒度/事务模式）、重构目标结构列、建模场景库（断路器/TCC/SAGA）、可观测性验收标准（日志/度量/追踪）、机制说理层（蜂群共识/验证器定位三原则/爬山法哲学/约束创造/满意化/不连续系统穷举/受控的失控/clockware-swarmware）；候选反模式四源-α/β/γ/δ（候选区，不正式编号）；版本号 41.2.0。
...
| check-role-dispatch.ts | 角色分派完整性校验（每阶段 S/V/G 各 ≥1 条；`--r3-enabled` 时 R ≥3 条；约束 #8） | 1-8 | 0=通过，1=校验失败，2=输入错误 |
...
| self-test.ts | 回归基线（252 条样本）；vitest 571 条（35 test files） | - | 0=通过，1=失败 |
| gate-enhancement.test.ts | 门禁增强回归测试（basePath/SD 覆盖/passed↔qualityLevel + codeModule 格式 + uat-path-mapping） | - | 0=通过，1=失败 |
| check-docs-consistency.ts | 活体文档一致性门禁（schema 清单 / run-log action 枚举 / targetKind / DoD 维度 / 操作行为 / 反模式 / exit-2 脚本数 / pre-push 项数 / glossary action / 资产计数 / design-docs / vitest-files / vitest-tests 实测用例总数） | - | 0=通过，1=不一致，2=输入错误 |
| check-samples-coverage.ts | samples 覆盖矩阵门禁（每个 fixture 被 self-test.ts 引用 + 每个子目录在 samples/README.md 矩阵声明，41.5.0 新增） | - | 0=通过，1=不一致，2=输入错误 |
