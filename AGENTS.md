...
- **Agent Personas（评审角色提示词）** 由 V 子代理在执行 `/wm review` 时按 `w-model-dev/references/agent-personas.md` 选用对应 Persona（code-reviewer / test-engineer / security-auditor / performance-auditor），Persona 文件本身是 Markdown，不调用 LLM；产出 JSON 须满足 `verifier-spec.md` §6 Schema。多角度分析时，R-lead / V-lead 按 `w-model-dev/references/subagent-persona-matrix.md` 从 `w-model-dev/subagent/`（28 个人格文件，分 engineering / testing / design / product / project 5 类）选用 persona 并行/串行分派。
...
- **错误结构全量归一化**：全仓 35 个脚本 exit 2 统一输出结构化错误——人类消息 `✗ [CATEGORY] ...` 走 stderr、`ERROR_JSON {...}` 摘要走 stdout（`lib/cli-error.ts`，6 类错误码）。详见 `w-model-dev/references/command-reference.md`「错误码与 ERROR_JSON 约定」节。
...
| `w-model-dev/scripts/` | 自包含门禁脚本（依赖 `tsx` runtime + devDeps：ajv / eslint-plugin-security，需 `npm install` 一次）：`logic/`（校验逻辑）+ `cli/`（CLI 入口）+ `lib/`（JSON/错误结构工具）。**完整 exit-2 脚本 35 清单见 §8「脚本导航表」与 [dispatch-matrix.md](w-model-dev/references/dispatch-matrix.md) §6 权威登记表**——新增 / 改名门禁脚本只须登记 dispatch-matrix 一处，`check-docs-consistency.ts` script-registry 检查兜底。`samples/`（fixture 样本）/ `__tests__/`（vitest 单元测试，55 个 .test.ts / 908 条 + README.md coverage 矩阵） | 阶段门 / 质量门 / 图谱门禁 / TLA+ 行为门禁 / 代码-TLA+ 一致性回归 / 签名链 / 归档完整性 / R3 预防性审查 / TLA+/BDD 同步 / 角色分派 / 状态机一致性 / 冰山扫掠检查点直接 `npx tsx` 执行 |
...
| `w-model-dev/schemas/` | JSON Schema (draft-07) 文件（23 份，全字段 description 自描述，含 evidence-manifest / evidence-provenance） | logic 层 schema 校验时自动加载；新增 .w-model/*.json 字段必先改 schema |
...
门禁脚本测试：
- `w-model-dev/scripts/__tests__/`：门禁脚本单元测试（vitest，55 个 .test.ts / 908 条）
...
npm run self-test                           # 260 条样本回归基线，退出码 0/1
...
npm run wm:verify-evidence-source -- <project-dir>  # producer+verify：校验并写入 source-bound provenance
npm run wm:export-evidence -- <project-dir> <output-dir>  # 显式导出脱敏 SHA-256 manifest 审计证据包；--verify 默认 package-only，传 --source-project 才 source-bound
...
| self-test.ts | 回归基线（260 条样本）；vitest 908 条（55 test files） | - | 0=通过，1=失败 |
...
| wm-verify-evidence-source.ts | source provenance producer+verify；校验当前 HEAD/run-log/passed gate/signature-chain 并原子写入 `.w-model/evidence-provenance.json`，不是只读 verify | - | 0=通过，1=源证据失败，2=输入错误 |
