...
| `w-model-dev/schemas/` | JSON Schema (draft-07) 文件（22 份，全字段 description 自描述，含 evidence-manifest） | logic 层 schema 校验时自动加载；新增 .w-model/*.json 字段必先改 schema |
...
门禁脚本测试：
- `w-model-dev/scripts/__tests__/`：门禁脚本单元测试（vitest，54 个 .test.ts / 886 条）
...
npm run wm:export-evidence -- <project-dir> <output-dir>  # 显式导出脱敏 SHA-256 manifest 审计证据包
...
| self-test.ts | 回归基线（260 条样本）；vitest 886 条（54 test files） | - | 0=通过，1=失败 |
