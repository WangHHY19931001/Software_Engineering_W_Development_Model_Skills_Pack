...
- **阶段 1 需求分析设计文档级产出**（第 37 轮新增）：主规格 + 6 独立产物文件（系统上下文/术语表/追踪矩阵/行为规格/DoD/UML 建模），主规格引用块串联，不再内联 feature 集（行为规格由 bdd .feature 文件 + bdd-manifest.json 承接）；门禁 `check-requirement-graph.ts` 新增 R7（追踪矩阵一致性）/R8（UML mermaid 配平）+ `--spec-dir`，`check-artifact-gate.ts --phase=1` 新增引用块/SSOT/DoD 结构校验。详见 [SSoT §3.4.35](./docs/skill-design-document_SSoT.md)
- **Phase 2 系统设计设计文档级结构**（第 38 轮新增）：6 独立产物 + 引用块 + 门禁核验（R9/R10 + 结构校验）。详见 [SSoT §3.4.36](./docs/skill-design-document_SSoT.md)
...
- [反例与失败模式](./w-model-dev/references/anti-patterns.md) - 44 条流程反模式（含 #10 编排者越权实施 / #11 ingestion 跳过图谱校验 / #12 A 自评收敛 / #13 信息流黑洞/奇迹/死模块放行 / #14 跳过 SANY 直接 TLC / #15 死锁/不变式违反放行 / #16 TLA+ 占位/简化/错误实现 / #17 TLA+ 建模不符需求/设计不回退 / #18 跳过 R 直接 S 返工 / #19 R 报告未 V 复审直接 S 修复 / #21 阶段级门禁跳过 / #22 角色越权 / #23 跨模块 store 误用 / #24 副作用时序不一致 / #25 JSON 文件 PowerShell 写入 / #26 RunLogEntry 与 EventIngress 字段混用 / #27 调测者简化行为 / #28 schema 前置校验缺失 / #29 BDD 建模与需求/设计/TLA+ 不符未回退 / #30 豁免审批跳步 / #31 归档完整性缺失 / #32 签名链断裂 / #33 跳过 R3 预防性审查 / #34 编排者漏派角色 / #35 self-as-verifier 产物混合 / #36 路由顺序错误 / #37 产物膨胀核心决策稀疏 / #38 修改前未查询 codegraph / #39 跳过 opsx 产物审查 / #40 opsx/S-tickets 职责混淆 / #41 加权平均掩盖单轴失败 / #42 S-fix / emergency-fix 后跳过 R3+V / #43 敏感信息写入状态文件/日志 / #44 跳过冰山扫掠直接放行；#20 见 [subagent-delegation.md](./w-model-dev/references/subagent-delegation.md)）+ L1~L4 实现层教训 + F1~F10 失败模式
