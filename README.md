...
- **阶段 1 需求分析设计文档级产出**（第 37 轮新增）：主规格 + 6 独立产物文件（系统上下文/术语表/追踪矩阵/行为规格/DoD/UML 建模），主规格引用块串联，移除内联 feature 集（bdd 承接）；门禁 `check-requirement-graph.ts` 新增 R7（追踪矩阵一致性）/R8（UML mermaid 配平）+ `--spec-dir`，`check-artifact-gate.ts --phase=1` 新增引用块/SSOT/DoD 结构校验。详见 [SSoT §3.4.35](./docs/skill-design-document_SSoT.md)
